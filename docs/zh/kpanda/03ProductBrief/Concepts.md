...
标签其实是一对 key/value，被关联到对象上，比如 Pod。标签的使用倾向于能够标示对象的特点，并且对用户而言是有意义的，但是标签对内核系统是没有直接意义的。

Label 是附着到特定对象上（例如 Pod）的键值对。可以在创建对象时指定，也可以在对象创建后指定，Label 的键值对系统本身并没有什么含义，仅对用户才有意义。
