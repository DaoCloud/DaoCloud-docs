...
无状态工作负载（Deployment）是运行在 Kubernetes 上的一种不保存任何数据和状态的应用。和其它工作负载类似，无状态工作负载也是由 Pod 构成。通过 Daocloud 容器管理平台，您可以基于所拥有的权限轻松管理多云多集群上的工作负载，包括对无状态工作负载的创建、更新、删除、弹性扩缩、重启、版本回退等全生命周期管理。
...
- 描述：输入工作负载的描述信息，内容自定义。字符数量应不超过 512 个。例如：这是一个无状态工作负载，主要用来运行 Nginx 服务。
