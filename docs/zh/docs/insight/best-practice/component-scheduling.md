...
本文档将重点探讨如何通过节点亲和性的配置策略，或者分配独立节点的方法，使得每个组件能够在适当的节点上运行，并避免资源竞争或争用，从而确保整个 Kubernetes 集群的稳定性和高效性。

## 1. 通过污点为 Insight 配置专有节点

由于 Insight Agent 包含了 Daemonset 组件，此处的方法指的是：Insight Dameonset 除外，其余组件均运行在专有节点。

该方式是通过为专有节点添加污点(taint)，并配合污点容忍度(tolerations) 实现。更多细节可以参考 [kubernetes 官方文档](https://kubernetes.io/zh-cn/docs/concepts/scheduling-eviction/taint-and-toleration/)。
...
为 `insight-system` 命名空间的 pod 都容忍 `inisght-only` 污点.
...
为了实现不同客户对 Insight 组件调度的灵活需求，Insight 分别提供了较为细粒度的 Label 用来实现不同组件的调度策略，以下是标签与组件的关系说明：
...
```bash
# 为 node8 添加标签，首选将 insight agent prometheus 调度到 node8 
kubectl label nodes node8 node.daocloud.io/insight-prometheus=true

# 去除 node8 的 node.daocloud.io/insight-prometheus 标签
kubectl label nodes node8 node.daocloud.io/insight-prometheus-
```

以下是 Insight Agent prometheus 组件在部署时默认的亲和性偏好：
...
affinity:
  nodeAffinity:
    ...
    - preference:
        matchExpressions:
        - key: node.daocloud.io/insight-prometheus #  # 首选将 insight agent prometheus 调度到带有 node.daocloud.io/insight-prometheus 标签的节点。
          operator: Exists
      weight: 2
    - preference:
        matchExpressions:
