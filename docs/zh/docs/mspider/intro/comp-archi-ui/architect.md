...
DCE 服务网格产品支持多种类型的网格实例管理能力，支持托管多集群网格进行云原生微服务治理能力；用户可将不同来源的集群接入托管网格中，进行统一的流量治理和安全治理等。
...
- 网格全局管理面
    ...
    同时，也负责与 DCE 其它模块的对接的工作，以将减少其它模块的适配成本。
...
- 可观测模块
    ...
    服务网格的可观测能力，借助 DCE 可观测性模块处理，服务网格通过接口调用的方式获取流量指标信息绘制拓扑图，并为用户提供各种维度的微服务监控图表。
...
- 应用工作台
    ...
    应用工作台是 DCE 的核心模块，用于提供给用户统一的应用编排、部署、流水线管理、CICD、应用监控、日志查询等能力。
