...
1. 屏幕提示流量透传设置成功。具体流量透传的效果，请参阅[下一节](#_3)。
...
=== "启用流量透传前"
    ...
    从浏览器访问 `http://10.64.30.142:31904/hello`，反复刷新，发现以下 2 条输出交替出现：
...
=== "启用流量透传后"
    ...
    从浏览器访问 `http://10.64.30.142:31904/hello`，反复刷新，发现输出只有：
    ...
    用 curl 查看 helloworld 的流量路由，目前流量走的是 k8s 默认的路由，未经过边车，流量已被透传。
