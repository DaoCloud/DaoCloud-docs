...
**模型管理**用于添加用户自定义模型或微调任务结果，并维护其元数据、部署模板与权重挂载信息，把可部署资产在平台里固化下来，减少重复配置、便于团队复用与后续快速部署。下文给出创建元数据及管理模板、权重文件的操作步骤。
...
部署模板用于预置模型资源配置与运行配置。部署自定义模型时，可直接选用模板，减少重复填写。
