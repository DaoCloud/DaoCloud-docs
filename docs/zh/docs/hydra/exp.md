...
体验中心支持使用预置的图像生成模型，也支持选择您自己创建的服务，另外在线体验的模型暂时支持 1 个。
...
| 参数名 | 参数说明 | 建议值 |
| ----- | ------- | ------ |
| 系统角色 | 定义模型的行为准则和背景信息，明确模型需要承担的职责和所扮演的角色。 | 例如“你是一个 AI 助手”。 |
| top_p | 控制输出文本的多样性，值越大，生成的文本越丰富多样。 | 0.8 |
| temperature | 值越高，输出结果越随机多样；值越低，输出会更加集中且确定。 | 0.7 |
| max_tokens | 模型可以生成的最大 Token 数，如设置为 0，则表示不限制。 | 普通聊天：500–800；<br>短文：800–2000；<br>代码：2000–3600；<br>长文：4000+ |

!!! note

    `temperature` 和 `top_p` 都能控制模型输出的随机性，但方式不同。

    - `temperature` 调节整体概率分布。数值越高，输出越随机、有创造性；数值越低，输出越稳定、确定性强。
      可以理解为“调整模型大胆程度的旋钮”。
    - `top_p` 限制参与采样的候选词范围。模型只在累计概率达到阈值的高概率词中选择下一个词，
      从而过滤掉低概率的“长尾”词。可以理解为“先把不靠谱的选项剔除再选词”。

    在工程实践中，通常二选一即可：

    - 想要稳定可控的输出时，优先使用 `temperature`
    - 希望输出自然流畅且不过于发散时，优先使用 `top_p`

    更多请参阅 [Hugging Face 文档的参数说明](https://huggingface.co/docs/transformers/)。
