...
1. 进入某一个多云实例后，在左侧导航栏中，点击`策略管理` -> `部署策略`，点击右上角的 `YAML 创建`按钮。
    ...
    ![点击按钮](../images/op01.png)

2. 在 `YAML 创建`页面中，输入正确的 YAML 语句后，点击`确定`。

    ![输入yaml](../images/op02.png)
...
1. 进入多云编排模块，点击某一实例——>策略管理——>差异化策略，本页面展示了当前实例内的所有差异化策略，点击列表上方的`创建差异化策略`按钮。
