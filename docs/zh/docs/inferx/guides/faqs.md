...
![gateway-api-helm-charts](imgs/gateway-api-helm-charts.png)
![install-gateway-api](imgs/install-gateway-api.png)
...
### 使用 HAMI & PD 分离情况下，Decode Pod 因 SidecarContainers 功能未开启导致无法调度，具体报错信息如下
```bash
...
### HTTPRoute 报错不支持 group `inference.networking.k8s.io` 或 `inference.networking.x-k8s.io`， 具体报错信息如下
```bash
