...
- [模型权重下载与配置](#模型权重下载与配置)
    - [模型权重管理方式](#模型权重管理方式)
    - [使用 Dataset 管理模型权重（推荐）](#使用-dataset-管理模型权重推荐)
        - [什么是 Dataset](#什么是-dataset)
        - [Dataset 安装](#dataset-安装)
            - [方案一：通过 DCE Hydra 内置 Dataset 能力](#方案一通过-dce-hydra-内置-dataset-能力)
            - [方案二：在 DCE 界面通过 Helm 安装 Dataset Addon](#方案二在-dce-界面通过-helm-安装-dataset-addon)
            - [方案三：手动 Helm 安装 Dataset](#方案三手动-helm-安装-dataset)
        - [模型权重下载](#模型权重下载)
            - [创建 Dataset 下载模型权重](#创建-dataset-下载模型权重)
            - [跨命名空间共享现有 Dataset](#跨命名空间共享现有-dataset)
        - [Dataset 与 PVC 对应关系](#dataset-与-pvc-对应关系)
        - [在 InferX 中使用 Dataset 模型](#在-inferx-中使用-dataset-模型)
    - [其他挂载方式](#其他挂载方式)
        - [方式一：通过 PVC 挂载共享存储](#方式一通过-pvc-挂载共享存储)
        - [方式二：通过自定义容器挂载模型目录](#方式二通过自定义容器挂载模型目录)
