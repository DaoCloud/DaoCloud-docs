...
可以使用全局管理提供的基于 OIDC 协议的身份提供商功能，而不必在 DCE 中为每一位组织成员创建用户名/密码。
您可以向这些外部用户身份授予使用 DCE 资源的权限。

具体操作步骤如下。

1. 使用具有 __admin__ 角色的用户登录 DCE。点击左侧导航栏底部的 __全局管理__ -> __用户与访问控制__ 。
...
3. 填写表单字段，建立与身份提供商的信任关系后，点击 __确定__ 。
    ...
    | 自动关联   | 开启后当身份提供商用户名/邮箱与 DCE 用户名/邮箱重复时将自动使二者关联 |

!!! note

    1. 当用户通过企业用户管理系统完成第一次登录 DCE 后，用户信息才会被同步至 DCE 的 __用户与访问控制__ -> __用户列表__ 。
...
1. 使用浏览器发起单点登录 DCE 的请求。
1. DCE 根据登录链接中携带的信息，查找 __全局管理__ -> __用户与访问控制__ -> __身份提供商__ 中对应的配置信息，
...
1. 浏览器响应后转发 OIDC 授权 Response 给 DCE。
1. DCE 从 OIDC 授权 Response 中取出 ID Token，并根据已配置的身份转换规则映射到具体的用户列表，颁发 Token。
1. 完成单点登录，访问 DCE。
