...
DCE 平台存在三种角色范围，能够灵活、有效地解决您在权限上的使用问题：
...
而不能具体到某一个集群或某一个工作空间。DCE 提供了 5 个预置的、用户可直接使用的平台角色：
...
同时，DCE 还支持用户创建自定义平台角色，可根据需要自定义角色内容。
...
获得该角色权限的用户只能管理该工作空间，而无法访问其他工作空间。DCE 提供了 3 个预置的、用户可直接使用的工作空间角色：
...
同时，DCE 还支持用户创建自定义工作空间角色，可根据需要自定义角色内容。如创建一个工作空间角色，
...
同时，DCE 还支持用户创建自定义文件夹角色，可根据需要自定义角色内容。
