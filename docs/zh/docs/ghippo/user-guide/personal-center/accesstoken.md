...
登录 DCE 5.0，在右上角的下拉菜单中找到`个人中心`，可以在`访问密钥`页面管理账户的访问密钥。
...
在访问 DCE 5.0 openAPI 时，在请求中加上请求头 `Authorization:Bearer ${token}` 以标识访问者的身份，其中 `${token}` 是上一步中获取到的密钥，具体接口信息参见[接口文档]()。
