...
- **新增** 支持短信功能通过 webhook 对接
- **新增** 增加日语 i18n 支持
- **修复** 修复 smtp 邮箱用户名密码不能保存为空
- **优化** 显示用户时包含 firstname/lastname 信息
- **优化** 优化 SMTP 邮箱连接测试，支持 Plain、Noauth、Login 多种加密方式自动适配
...
- **新增** (CSP) 用户注册登录功能实现 keycloak 初始化配置的自动化
- **新增** 审计日志支持多条件模糊搜索功能
- **修复** 邮箱注册邮件发送失败问题
...
- **新增** 集成可观测链路拓扑功能升级
- **修复** 用户的 OpenAPI 的 Token 不包含用户组的权限的问题
- **修复** LDAP 用户不能修改用户语言的问题
...
- **新增** 公有云个人用户实名认证
- **新增** 添加自定义角色增删改审计日志
- **修复** `ListGProducts API` 登录态检查问题
- **修复** 保存 SMTP 时，审计日志未记录的问题
- **修复** 用户授权主体列表搜索和用户组授权主体列表搜索规则不一致问题
- **修复** 全局管理和命名空间配额管理的样式不统一问题
- **修复** 工作空间中 vGPU 已分配显存不正确问题
...
- **修复** 工作空间 vGPU 已分配配额计算问题
- **修复** 用户列表搜索用户名包含 `_` 无返回的异常问题
- **修复** 部分审计日志资源名称为空的问题
- **修复** 审计日志用户名显示异常的问题
...
- **修复** 审计日志 Create-User 对应的资源名称显示异常问题
...
- **修复** user ldap filter 重新设置为空同步后相应字段没有清空的问题
...
- **新增** **全局管理** -> **运营管理** -> **计费配置** 中安装 insight-angent 提示
...
- **新增** 资源配额支持限制 GPU
- **新增** 支持 DCE 的登录用户才能打开 Insight 的 Grafana 等组件的界面
