...
- **新增** 新增 OAuth2 Identity Provider 通用插件
- **新增** LDAP 配置增加 username、userLdpaFilter 字段
- **新增** __运营管理__ 计费配置添加 安装 insight-angent提示

### 优化

- **优化** 创建用户，用户名前后有空格时自动去除后保存
- **优化** 绑定资源搜索优化
- **新增** SDK 方法 ListWorkspaceUsersByPermission()

### 修复

- **修复** 修复自定义角色授权用户后能看到所有工作空间问题
- **修复** 修复浏览器关闭后再打开后可观测性等页面出现 RBAC Access Denied 页面
- **修复**  __运营管理__ 修复报表导出字段名称与页面不一致问题
