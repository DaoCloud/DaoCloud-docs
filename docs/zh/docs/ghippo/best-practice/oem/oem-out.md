# 如何将 DCE 集成到客户系统（OEM OUT）

OEM OUT 是指将 DCE 作为子模块接入其他产品，出现在其他产品的菜单中。
用户登录其他产品后可直接跳转至 DCE 无需二次登录。实现 OEM OUT 共分为 5 步，分别是：
...
1. 部署 DCE（假设部署完的访问地址为 `https://10.6.8.2:30343/`）

1. 客户系统和 DCE 前可以放一个 nginx 反代来实现同域访问，
   __/__ 路由到客户系统， __/dce5 (subpath)__ 路由到 DCE 系统， __vi /etc/nginx/conf.d/default.conf__ 示例如下：
...
1. 假设 nginx 入口地址为 10.6.165.50，按[自定义 DCE 反向代理服务器地址](../../install/reverse-proxy.md)把
   DCE_PROXY 反代设为 `http://10.6.165.50/dce5`。确保能够通过 `http://10.6.165.50/dce5`访问 DCE。
...
将客户系统与 DCE 平台通过 OIDC/OAUTH 等协议对接，使用户登录客户系统后进入 DCE 时无需再次登录。
...
对接完成后，DCE 登录页面将出现 OIDC（自定义）选项，首次从客户系统进入 DCE 时选择通过 OIDC 登录，
后续将直接进入 DCE 无需再次选择。
...
对接导航栏是指 DCE 出现在客户系统的菜单中，用户点击相应的菜单名称能够直接进入 DCE。
