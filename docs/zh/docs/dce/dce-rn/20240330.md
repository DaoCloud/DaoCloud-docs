...
- **修复** Cocker Registry **镜像删除** 会出现删除镜像失败
- **修复** 普通租户（wsadmin 权限）创建镜像回收规则失败

### 容器网络 v0.13.0

- **修复** 删除 SpiderMultusConfig 资源之前，遍历所有 SpiderIPPool 资源，要求用户提前将 `SpiderIPPool.Spec.MultusName` 字段中绑定的 `SpiderMultusConfig` 字符串移除
