...
- 可观测性 Insight
  ...
  在 DCE 5.0 v0.34.x 版本中，Insight 已将 Jaeger 从 v1 升级至 v2，并对整体部署架构进行了调整。升级到该版本之前，请在产品清单文件 manifest.yaml 
  中的 `components.insight.variables` 下添加配置：`jaeger.collector.enabled: false`。
