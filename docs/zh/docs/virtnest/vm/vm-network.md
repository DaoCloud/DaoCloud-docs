...
3. 添加网卡
    ...
    2. 在 __网卡 IP 池配置__ 弹窗中选择 Multus CR，并为网卡选择 IPv4 池。点击 __添加__ 可增加 IP 池，并可通过上下箭头调整 IP 池的顺序。
       
        ![添加网卡](../images/createvm-net03.png)

    3. 完成配置后，可在网络列表中查看各网卡使用的 Multus CR 和 IP 池。

        !!! warning

            为避免网络通信异常，不同网卡使用的 IP 池必须属于不同网段。请勿为不同网卡重复选择相同的 Multus CR 和 IP 池组合。

        ![网卡网络配置](../images/createvm-net04.png)
