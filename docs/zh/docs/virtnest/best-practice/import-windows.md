...
将外部平台的虚拟机导入到 DCE 5.0 的虚拟化平台中时，需要根据虚拟机的启动类型（BIOS或UEFI）进行相应的配置，以确保虚拟机能够正确启动和运行。
...
1. 准备 window.yaml 文件,注意以下配置项

    <1>: 引导 Virtio 驱动的 PVC
    <2>: 磁盘总线类型,根据引导类型设置为 sata 或 virtio
    <3>: 如果使用 UEFI ,需要添加 UEFI 配置

??? note "点击查看 window.yaml 示例"
...
- 使用RDP客户端连接到虚拟机。输入默认账号 administrator 和密码 dangerous!123 进行登录。
