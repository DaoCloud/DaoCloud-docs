...
1. 从左侧导航栏，依次点击 __虚拟机__ -> __虚拟机__ ，进入 __虚拟机管理__ 页面。在虚拟机列表页面，点击创建虚拟机-选择模板创建虚拟机。
...
可以基于虚拟机模板批量创建配置相同的虚拟机。

1. 从左侧导航栏依次点击 __虚拟机__ -> __虚拟机模板__，进入虚拟机模板列表。

2. 找到目标虚拟机模板，点击右侧的 __┇__，选择 __批量创建虚拟机__。

    ![批量创建虚拟机](../images/batch-create-vm-template.png)
...
3. 填写批量创建参数：
    ...
    - 集群：选择虚拟机运行的集群。
    - 命名空间：选择虚拟机所属的命名空间。
    - 创建数量：填写需要创建的虚拟机数量，最多支持 10 个。
    - 自定义密码：设置批量创建的虚拟机登录密码。
    - 存储池：选择虚拟机使用的存储池。
    - PVC 读取模式：选择 PVC 的读取模式。

4. 点击 __确定__。系统将同时创建指定数量的虚拟机，并按照“前缀-序号”的格式自动命名，例如 `demo-1`、`demo-2`、`demo-3`。创建成功后，虚拟机将进入运行状态；虚拟机名称的总长度不能超过 60 个字符。

批量创建的虚拟机使用相同的模板配置以及批量创建页面中填写的集群、命名空间、存储和网络等配置。网络支持 `masquerade` 和 `bridge` 两种模式，IP 地址均由系统从 IP 池中随机分配；如果 IP 池中没有可用 IP，批量创建将失败。
