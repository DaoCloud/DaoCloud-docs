...
- **优化** 边缘节点接入流程，接入工具使用 keadm，并重新构建边端安装包
- **优化** 边缘单元删除 kant-workeragent 的相关配置，使用默认配置安装/卸载此组件，升级交给 helm 应用管理
- **修复** 批量工作负载临时过滤除 Deployment 以外的资源配置问题
