...
- **新增** 支持volcano binpack 和 优先级抢占策略，可参考操作文档在容器管理平台使用该策略
- **新增** 支持沐曦 GPU 卡的使用
- **新增** GPU监控面板中增加GPU利用率指标
- **新增** NS添加配额和使用量显示 
- **新增** 支持平台下发的kubeconfig可以永久有效
- **新增** 自定义角色支持工作空间与命名空间之间的权限映射
- **新增** 通过快照创建PVC，支持用户自己选择sc

#### 优化

- **优化** GPU整卡模式下，可用 GPU 算力提示文字优化
- **优化** 优化命名空间绑定工作空间绑定后页面未显示绑定的工作空间，需要手动刷新页面问题
- **优化** 兼容 volumeMode 为 block的 pv
- **优化** addon合并镜像架构后未清理中间镜像

#### 修复

- **修复** 修复vgpu设置算力100，但仪表盘pod的算力使用率显示为0%的问题
- **修复** 修复GPU operator未开启driver选项导致不显示GPU模式切换功能问题
- **修复** 修复仪表盘 GPU 显示个数超过实际值问题
- **修复** 修复mig mixed模式下，节点详情页的GPU使用率数据显示有误
- **修复** 【1000+集群】修复大规模场景Binding syncer 内存占用过大问题
- **修复** 【1000+集群】修复给每一个集群绑定 ws 时，redis 的链接数突增，且长时间不会关闭问题
- **修复** 【1000+集群】修复大规模场景中出现的工作空间共享资源权限错乱问题
- **修复** 修复集群绑定工作空间，没有出现审计日志问题
- **修复** 修复安全管理中的 合规性扫描 的集群出现虚拟机集群的问题
- **修复** 修复卸载helm应用，偶发出现2个卸载job问题
- **修复** 修复安装器 v0.19.0升级至v0.20.0 安装metrics-server失败问题
- **修复** 修复创建集群时点击检测再取消，检测任务未取消，后续创建时点击检测都提示在检测中
- **修复** 修复容器管理-节点管理--global集群无法接入和移除节点问题
- **修复** 修复卸载集群，一直在删除中，实际已卸载失败问题
- **修复** 修复解除集群时 kpanda-system 下 finalizers 资源未删除，导致命名空间 kpanda-system无法删除问题
- **修复** 修复dce5环境中已经做了kubean 向下兼容包，低版本集群应该支持卸载。但是并没有删除成功问题
- **修复** 修复备份恢复触发时间显示有误问题
- **修复** 修复集群巡检配置，定时任务小时数，触发时间与配置时间不符问题
...
- **修复** GPU driver Pod 对应进程的 GPU 显存使用和应用里查看的信息显示不一致问题
- **修复** vGPU 模式的情况下，关机重启 vm 后，变成整卡模式问题
- **修复** npu 开启虚拟化之后节点上的 lable 丢失问题
- **修复** GPU Pod 仪表盘 Pod GPU 算力使用率显示有误问题
- **修复** GPU Pod 仪表盘中的 GPU Pod 显存使用量/使用率显示有误问题
- **修复** 使用 Ascend 卡创建工作负载，监控指标没有数据展示问题
- **修复** vGPU 修改 deviceSplitCount 未生效问题
- **修复** vGPU 模式下，显存如果填写 5000，更新的时候页面不能回显问题
- **修复** 集群设置菜单下的 Addon 插件 GPU 类型下拉框中未展示 mig 相关的信息问题
- **修复** 有 GPU 的节点重启或者驱动 driverpod 升级 vgpu-device-plugin 会持续 crash 问题
- **修复** GPU 配额中 GPU 显存单位和内存配额单位不一致问题
- **修复** 更新 Go dependency 失败，OTel 依赖冲突问题
- **修复** cloudshell CRD 超时页面终端仍然提示可以重连问题
- **修复** clusteradmin 权限的用户（无全局服务集群的权限），查看最近操作 -> 集群操作页面的立即前往，
  可以直接进入到全局服务集群的详情页问题
- **修复** 使用 Namespace Admin 权限的用户查看工作负载详情页容器配置的基本信息，存在接口报 403
- **修复** NS 的 Quota 展示出来了，但是编辑界面的数据为空问题
- **修复** 容器组详情页的镜像地址和工作负载详情页显示的镜像地址不一致问题
- **修复** 更新 insight-ui 无状态负载，进入到容器配置- 健康检查页面，查看端口信息未正常展示问题
- **修复** 节点 kubeproxy 版本显示异常问题
- **修复** DCE5-0.19 接入节点或者创建集群的时候，对 Ubuntu 22.04 识别系统代号识别有问题
- **修复** Helm 创建应用状态处于未知状态，点击应用详情报错问题
- **修复** 工作集群创建 Helm 应用时前端显示应用的初始状态为失败状态问题
- **修复** 更新 Helm 应用时，就绪等待配置未记录创建时的状态问题
- **修复** 开启就绪等待接入子集群安装 Helm 应用失败，Helm 应用状态始终处于安装中问题

**已知问题**

- **修复** kpanda 前后端版本需保持一致，否则 vGPU 模式的情况下，关机重启 VM 后，变成整卡模式
...
- **修复** Kpanda 数据库连接问题
- **修复** NS Admin 角色 PV/PVC 权限问题修复
- **修复** 修改集群的基础配置后，egress 端口被刷新，导致数据流中断，controller-manager 工作异常问题
- **修复** 在工作负载详情的访问方式页面执行重启工作负载操作，service 接口报 404 问题
- **修复** addon 没有 cro-operator 对应的离线包问题
- **修复** Ubuntu 内核自动更新升级时可能导致的系统在不经意间被重启问题
- **修复** 安装使用 MIG 模式，偶尔在节点上显示整卡模式问题
- **修复** GPU 虚拟化后，内存超配功能不可用问题
- **修复** 调整 GPU 调度策略偶发调度策略切换失败问题
...
- **优化** 节点详情页查看 GPU 资源分配情况的快捷链接，跳转到的监控页面未集成 GPU 配额问题优化
...
- **修复** cloudtty Pod 无法使用带证书的 kubeconfig 访问子集群问题
- **修复** 多架构融合，存在 addon 包里没有对应的镜像，导致服务不能正常运行问题
- **修复** Deployment 实例列表显示了非当前 Deployment 的 Pod 问题
- **修复** GPU node 仪表盘中的显存分配率和节点上的不一致问题
- **修复** GPU 节点的标签显示有误问题
- **修复** LoadBalancer 类型 service，做更新操作，修改 lb IP 地址后，查看 service 详情，会概率出现 nodeport 访问方式的展示问题
- **修复** 创建工作负载 mig 模式配置多类型 GPU 规格问题
- **修复** MIG Mixed 模式下，Deployment 配置多个不同类型 GPU 规格配置，配置与实际不符问题
...
- **优化** service 更新时，loadBalancerIP 不可用情况下，提示优化
- **优化** binding-syncer 支持 lease 选举
- **优化** kpanda 整卡模式的算力单位描述统一
- **优化** GPU 监控指标支持中英文切换
- **优化** vGPU 安装建议默认关闭 servicemonitor 提示优化
...
- **修复** 优化用户的 workspace admin 权限同步到 kpanda 中 cluster admin 权限时页面无显示的问题
...
- **修复** 大规模场景下全局管理集群中的容器管理下的工作负载等页面 ns 接口加载缓慢，导致页面使用卡顿
