...
- **修复** 修复 vGPU 设置算力 100，但仪表盘 Pod 的算力使用率显示为 0% 的问题
- **修复** 修复 GPU Operator 未开启 Driver 选项导致不显示 GPU 模式切换功能问题
- **修复** 修复仪表盘 GPU 显示个数超过实际值问题
- **修复** 修复 MIG Mixed 模式下，节点详情页的 GPU 使用率数据显示有误
- **修复** 在 1000+ 集群中，修复大规模场景 Binding Syncer 内存占用过大问题
- **修复** 在 1000+ 集群中，修复给每一个集群绑定工作空间时，Redis 的链接数突增，且长时间不会关闭问题
- **修复** 在 1000+ 集群中，修复大规模场景中出现的工作空间共享资源权限错乱问题
- **修复** 修复集群绑定工作空间，没有出现审计日志问题
- **修复** 修复安全管理中的 合规性扫描 的集群出现虚拟机集群的问题
- **修复** 修复卸载 Helm 应用，偶发出现 2 个卸载 Job 问题
- **修复** 修复安装器 v0.19.0 升级至 v0.20.0 安装 metrics-server 失败问题
- **修复** 修复创建集群时点击检测再取消，检测任务未取消，后续创建时点击检测都提示在检测中
- **修复** 修复容器管理 -> 节点管理中，全局服务集群无法接入和移除节点问题
- **修复** 修复卸载集群，一直在删除中，实际已卸载失败问题
- **修复** 修复解除集群时 `kpanda-system` 下 finalizers 资源未删除，导致命名空间 `kpanda-system` 无法删除问题
- **修复** 修复 DCE 5.0 环境中已经做了 kubean 向下兼容包，低版本集群应该支持卸载，但是并没有删除成功问题
- **修复** 修复备份恢复触发时间显示有误问题
