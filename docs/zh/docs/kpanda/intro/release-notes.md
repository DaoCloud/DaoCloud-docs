...
- **新增** 支持阿里 PPU 异构 GPU 卡
- **新增** HAMI 社区版升级 2.9.0 版本
- **新增** etcd 备份支持对接 https 的对象存储
- **优化** 命名空间列表页添加列GPU用量与显存用量
- **优化** 指定 GPU 卡需要 insight-agent 已安装
- **优化** 节点检查超时优化
...
- **新增** 支持 GPU 管理
- **优化** 创建集群/接入节点时，批量上传节点的使用说明
- **优化** 创建集群失败时集群列表的状态显示
- **修复** Helm 应用更新时，YAML 数据异常问题
- **修复** GPU 配置中最大使用的显存大小错误问题
- **修复** 开启 UseTokenInCloudShell 特性后无法打开控制台进入 pod 问题
- **修复** 创建集群失败重试/接入节点时 os_package_repo 为必填项问题
- **修复** ubuntu22.04 os 集群添加节点开启内核调优后接入节点报错问题
- **修复** etcd 备份无法使用问题
- **修复** 无状态负载的容器组中无法正常下载文件问题
- **修复** 安装 Helm应用时报错问题
- **修复** Helm 应用卸载时，日志显示信息错误问题
- **修复** 配额管理中无法更新 GPU 配额问题
...
- **修复** 适配 kubean/kubespray 上游移除了组件版本中的 v 字符 
- **修复** 集群中使用专属的 resolv.conf 
- **修复** elm 应用状态失败时，再次更新提示有正在运行的 helm operation 无法
- **修复** cloud-tty 内存泄漏问题。
- **修复** 安装 helm 应用时，如果安装中或者失败时 helm 仓库信息丢失。
- **修复** 训练任务完成后 gpu 模式无法切换。
- **修复** 权限设置跳转链接问题。
- **更新** helm 应用。
- **优化** 调整 helm 应用失败后的重试次数为 1 次 (Issue #5017)
...
- **修复** 修复 metax-operator 离线安装之后，各个服务的镜像拉取报错问题
...
- **修复** DCE 环境中已经做了 kubean 向下兼容包，低版本集群应该支持卸载，但是并没有删除成功的问题
- **修复** 备份恢复触发时间显示有误的问题
