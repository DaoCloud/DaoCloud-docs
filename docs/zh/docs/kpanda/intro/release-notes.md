...
- **新增** 实现clusterpedia 对接 OTEL Tracing
...
- **优化** kpanda加入大量pod集群后pod列表刷新时间太长了
- **优化** 升级gpu-operator 到 v23.9.0，缩小与社区版本差距
- **优化** 备份整个ns（ns下存在cr以及pvc相关的内容），备份成功后进行恢复，显示部分成功但是看不出哪些是成功的哪些是失败的

#### 修复

- **修复** 修复 addon 生命周期管理过程导致权限泄漏
- **修复** 修复 定时伸缩的任务名称相同时，功能失效
- **修复** 修复离线环境，安装了kubernetes-cronhpa-controller之后，页面无法检测到已经安装
- **修复** 修复ListPodsByNodeOrigin接口默认未按创建时间排序
- **修复** 修复ListContainersByPod接口小概率返回的container列表为空
- **修复** 修复流水线中的scheduled_e2e任务执行过程中报错提示cluster member1 not exist，后续tests测试用例没有得到执行
- **修复** 修复Data Collection按照中文搜索无响应
- **修复** 修复命名空间-资源限额不生效和更新异常问题
- **修复** 修复工作负载-负载监控的读写数据永远为空
- **修复** 修复gpu-operator 镜像没有全部离线问题
- **修复** 修复Kpanda 的 helm 安装会把集群管理权限泄露给普通用户
- **修复** 修复创建计划把备份数据卷打开后，备份详情里数据卷备份显示关闭
- **修复** 修复没有权限的用户，通过接口也能获取到其他集群的应用备份计划问题
- **修复** 修复velero版本与dce4的k8s版本不兼容
- **修复** 修复【大规模场景】用户和用户组列表接口加载缓慢
- **修复** 修复【大规模场景】clusterpedia接口超时报错，云边协同状态无法正常获取，导致无法使用
- **修复** 修复【大规模场景】命名空间没有展示绑定的所有命名空间
- **修复** 修复【大规模场景】全局管理集群中的容器管理下的工作负载等页面ns接口加载缓慢，导致页面使用卡顿
...
- **修复** 修复工作空间的可分配资源额度超过总配额
- **修复** 修复 SQL 注入的安全漏洞
- **修复** 修复创建 UOS 系统集群的失败问题
...
- **新增** __Helm Repo__ 密码连通性校验，支持跳过 TLS 证书认证。
- **新增** 全局服务机器的工作节点扩容。
...
- **新增** 集成自研开源存储组件 Hwameistor，支持在 __容器存储__ 模块查看本地存储资源概览等信息。
...
- **优化** 集群详情页，集群切换的使用体验。
- **优化** 工作负载状态显示，增加 __已停止（Stopped）__ 状态。
...
- **修复** 创建工作负载，镜像拉取失败问题。
- **修复** 定时的备份策略，无法执行 __立即执行__ 操作的问题。
- **修复** 修改无资源限制的工作负载时，UI 会自动添加资源限制问题。
- **修复** 当 __工作空间__ 没有与任何用户进行绑定时，往这个 __工作空间__ 添加命名空间失败的问题。
- **修复** 修复绑定、解绑命名空间会导致命名空间注解消失的问题。
- **修复** 修复创建集群使用 __kube-vip__ 策略不生效的问题。
- **修复** 创建集群设置 __ntp servers__ 为空时，将清空主机已有 __ntp__ 地址问题。
