...
!!! note
    ...
    本文仅针对离线模式下，使用 DCE 平台所创建的工作集群的 Kubernetes 的版本进行部署或升级，
    不包括其它 Kubernetes 组件的部署或升级。

本文适用以下离线场景：

- 用户可以通过以下操作指南，部署 DCE 平台所创建的非界面中推荐的 Kubernetes 版本。
- 用户可以通过制作增量离线包的方式对使用 DCE 平台所创建的工作集群的 Kubernetes 的版本进行升级。
