...
当系统内置的 CPU 和内存两种指标不能够满足您业务的实际需求时，您可以通过配置 ServiceMonitoring 来添加自定义指标，并基于自定义指标实现弹性伸缩。本文将介绍如何为工作负载配置基于自定义指标进行弹性伸缩。
...
若通过 insight 安装 Prometheus，则 serviceMonitor 上必须打上 operator.insight.io/managed-by: insight 这个label，通过其他方式安装则无需此label。
