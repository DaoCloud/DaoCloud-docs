...
本章节提供沐曦 Gpu-extensions、Gpu-operator等组件的安装指导和沐曦 GPU 整卡和 vGPU 两种模式的使用。

## 前提条件

1. 已在[沐曦软件中心](https://sw-download.metax-tech.com/software-list) 下载了安装所需的 tar 包，本文以 metax-gpu-k8s-package.0.7.10.tar.gz 为例。
2. 准备 Kubernetes 基础环境

## 组件介绍

Metax 提供了两个 helm-chart 包，一个是 Metax-extensions, 一个是 GPU-opeator，根据使用场景的不同可选择安装不同的组件。
...
3. DCE5.0 平台安装 metax-gpu-extensions
  

部署成功之后，node上可以查看到资源
![](../images/metax-node.png)

  
4. 修改DCE5.0 的配置，在kpanda-global-cluster 集群中的kpanda-system namespace下修改 gpu-type-config configmap，修改 gpu-type.json，添加如下内容。
...
5. 修改成功之后就可以在node上看到带有 `Metax GPU` 的标签
...
1. `metax-operator`、`gpu-label`、`gpu-device` 、`container-runtime` 这几个组件镜像要带 `amd64` 的后缀。
  
2. `metax-maca` 组件的镜像不在`metax-k8s-images.0.7.13.run` 包里面，需要单独下载 `maca-mxc500-2.23.0.23-ubuntu20.04-x86_64.tar.xz` 类似的这个镜像，`load` 之后重新修改 `metax-maca` 组件的镜像即可。
  
3. `metax-driver` 组件的镜像需要 `https://pub-docstore.metax-tech.com:7001`  这个网站上下载 `k8s-driver-image.2.23.0.25.run` 这个文件，然后执行 `k8s-driver-image.2.23.0.25.run push {registry}/metax` 把镜像推送到镜像仓库。推送之后修改 `metax-driver` 组件的镜像地址即可。
...
5. 进入容器中执行：`python /workspace/inference/vllm/examples/offline_inference_ollam3.py`
...
1. Nodes 下针对每个 node 配置 vfnum 即可; node 上的 Capacity 值在未开启虚拟化时与 GPU 物理设备数量相等；开启虚拟化后，预期资源数据为 N(gpu) * M(vfnum)
...
- 用 `mx-smi --show-vbios` 看下 `vbios` 是否支持虚拟化
...
4. 进入容器中执行：`python /workspace/inference/vllm/examples/offline_inference_ollam3.py`
...
|     |     |     |
| --- | --- | --- |<colgroup><col width="100"><col width="226"><col width="234"></colgroup>
| 命令  | 说明  | 条件  |
| mx-smi | 沐曦管理工具 | 安装驱动 |
|     |     |     |
...
1. ./metax-k8s-images.0.7.10.run 这个命令只能使用docker，暂不兼容 ctr 等其他 containerd 的命令行工具。
  
2. 镜像推送到镜像仓库之后镜像tag是带芯片架构信息, 但是拉取的时候又不带
...
node-feature-discovery 没有把CRD部署起来：
