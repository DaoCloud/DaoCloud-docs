...
在 DCE 容器管理模块中，[集群角色](cluster-role.md)分四类：全局服务集群、管理集群、工作集群、接入集群。
...
推荐使用 [DCE 支持的操作系统](../../../install/commercial/deploy-requirements.md)来创建集群。
...
- 推荐 Kubernetes 版本 1.29.5，具体版本范围，参阅 [DCE 集群版本支持体系](./cluster-version.md)，
  目前最新版 DCE 支持自建工作集群版本范围在 `v1.28.0-v1.30.2`。如需创建低版本的集群，请参考[集群版本支持范围](./cluster-version.md)、[部署与升级 Kubean 向下兼容版本](../../best-practice/kubean-low-version.md)。
...
!!! tip
    ...
    如需停止自动更新集群证书以及如需配置证书签发有效期，可以在高级参数中进行配置(容器管理 kpanda v0.41 及以上版本支持配置证书的签发有效期)：
