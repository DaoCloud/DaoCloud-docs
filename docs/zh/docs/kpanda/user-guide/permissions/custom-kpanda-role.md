...
- 使用 admin 或者 cluster admin 权限的用户[创建无状态负载](https://docs.daocloud.io/kpanda/user-guide/workloads/create-deployment.html)

  ![image-20240514112742395](./images/image-20240514112742395.png)

- 授权 ns-viewer ，用户有该 namespace ns-view 权限
  ![image-20240514113009311](./images/image-20240514113009311.png)

- 切换登陆用户为 ns-viewer，打开控制太获取 ns-viewer 用户对应的 token，使用 curl 请求去删除上述的 deployment nginx，发现无删除权限
  ```bash
...
- 在 global 集群上创建如下 cluster role，yaml 如下所示
  ```yaml
...
- 等待 kpanda 控制器添加用户创建权限到内置角色中 ns-view 中，可查看对应内置角色如是否有上一步新增的 rules
  ```
...
- 再次使用 curl 请求去删除上诉的 deployment nginx，可成功删除
