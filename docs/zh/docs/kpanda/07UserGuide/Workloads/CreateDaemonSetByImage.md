...
3. 依次填写[基本信息](CreateDaemonSetByImage.md#_3)、[容器配置](CreateDaemonSetByImage.md#_4)、[服务配置](CreateDaemonSetByImage.md#_5)、[高级配置](CreateDaemonSetByImage.md#_6)后，在页面右下角点击`确定`完成创建。
...
为守护进程创建[服务（Service）](../ServicesandRoutes/CreatingServices.md)，使守护进程能够被外部访问。

1. 点击`创建服务`按钮。

    ![服务配置](../../images/daemon03.png)

2. 配置服务参数，详情请参考[创建服务](../ServicesandRoutes/CreatingServices.md)。
