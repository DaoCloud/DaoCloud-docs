...
*[SeaweedFS]: 兼容 S3 协议的开源分布式对象存储系统，适合海量小文件和云原生非结构化数据场景。
...
- **新增** 支持查看当前用户在所有可见工作空间中的 Elasticsearch 实例
- **修复** 升级 Go 版本修复安全漏洞
- **优化** 适配实例列表筛选条件

### Kafka v0.29.0

- **新增** 支持查看当前用户在所有可见工作空间中的 Kafka 实例
- **修复** 修复 Addon 导入失败的问题
- **优化** 适配实例列表筛选条件

### MinIO v0.24.0

- **新增** 支持查看当前用户在所有可见工作空间中的 MinIO 实例
- **修复** 升级 Go 版本修复安全漏洞
- **修复** 实例状态无法反应 pod 状态问题
- **优化** 适配实例列表筛选条件

### MongoDB v0.19.0

- **新增** 支持查看当前用户在所有可见工作空间中的 MongoDB 实例
- **新增** 支持记录修改备份配置的审计日志
- **修复** 通过禁用 `mongod` zlib 压缩缓解 CVE-2025-14847
- **优化** 适配实例列表筛选条件，并统一工作空间过滤条件处理

### MySQL v0.29.0

- **新增** 支持查看当前用户在所有可见工作空间中的 MySQL 实例
- **修复** 资源使用率展示统一为 `usage/limit` 口径并修正单位
- **修复** 修复 MGR operator arm 镜像问题
- **修复** 修复 phpmyadmin yaml env 中引用密码的问题
- **优化** 更新接口增加参数校验，并完善查询后的校验流程
...
- **修复** 修复自动备份集恢复失败的错误
- **修复** 修复 CronJob 时区问题
- **修复** 升级 Go 版本修复安全漏洞 CVE-2025-68121
- **优化** 适配实例列表筛选条件，并统一工作空间过滤条件处理

### RabbitMQ v0.31.0

- **新增** 支持查看当前用户在所有可见工作空间中的 RabbitMQ 实例
- **优化** 适配实例列表筛选条件

### Redis v0.30.0

- **新增** 支持查看当前用户在所有可见工作空间中的 Redis 实例
- **修复** 分片槽不连续时拓扑图展示问题
- **修复** 升级 helm-kubectl 修复安全漏洞
- **优化** 适配实例列表筛选条件，并统一工作空间过滤条件处理
  
### RocketMQ v0.18.0

- **新增** 支持查看当前用户在所有可见工作空间中的 RocketMQ 实例
- **优化** 适配实例列表筛选条件，并统一工作空间过滤条件处理
