# Elasticsearch 集群规格和容量规划优化

## 前言

当您准备创建 Elasticsearch 时，建议提前评估集群所需的资源情况，一般情况下我们需要评估所需磁盘容量、集群规格、存储的文件类型、大小和数量等。通过评估这些资源。预先规划好 Elasticsearch 集群配置，可以降低在后续使用过程中发生不必要的问题和风险。

> 一个重要的知识: ES 集群的物理磁盘容量 !=  ES 集群可使用数据空间大小

本文是基于实际测试结果和用户使用经验，为大家提供了相对通用的评估方法；另外业务是瞬息万变的，所以本文的目的是在您创建 Elasticsearch 时给出一个相对合适的建议而已。实际 DCE5 也提供了优化的编辑和管理能力，方便大家后续对资源进行升级。

## 主要评估关键点

要进行Elasticsearch的容量规划，需要根据以下因素进行评估：

- 使用场景：主要使用的业务场景，不同的场景的下的数据量不大的
...
在存储容量规划的开始时，我们需要预先确认两个 `使用场景` 和 `数据要求` 规划：
...
- 索引开销：可以使用 `_cat/indices?v` API 和 `pri.store.size` 值计算确切的开销计算，通常比源数据大10%（_all参数等未计算）
- 操作系统预留空间：默认操作系统会保留5%的文件系统供您处理关键流程、系统恢复以及防止磁盘碎片化问题等
- `Elasticsearch` 内部开销：段合并、日志等内部操作，一般预留20%
...
如果你有关于 Elasticsearch 如何进行容量规划的需求，欢迎评论讨论。