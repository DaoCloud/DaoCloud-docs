...
2. pvc扩容日志
```shell
### elasticsearch-data-mcamel-common-es-cluster-masters-es-masters-0 扩容日志信息
...
查看业务索引模板中使用的别名 `rollover_alias对应值`
![image](../images/faq-es-2.png)

临时处理方式【进入es pod容器内执行以下脚本】

1. 修改TEMPLATE_NAME对应值

2. 修改INDEX_ALIAS对应值

3. 需要进入elasticsearch pod中执行该脚本

4. 修改里面elastic用户的密码值(ES_PASSWORD=xxxx)
```shell
#!/bin/bash
...
- 方式一：产生 429 错误的原因是 `Elasticsearch` 写入并发过大，`Elasticsearch` 来不及处理导致，可以适当降低写入并发并控制写入量。

- 方式二：在资源允许的情况下，可以适当调大队列大小
```shell
nodeSets:
  - config:
