...
- **新增** 流水线日志支持配置时区来显示日志时间
- **新增** 流水线列表支持更多排序方式
- **新增** 新增 FeatureGate `PipelineRerunConfirm`，支持配置流水线重新运行时的二次确认功能
- **新增** 通过 API 运行流水线时，未传参数名则使用流水线默认参数
- **修复** 通过 API 运行流水线时，请求参数未传值导致 choice 类型参数值展示错误的问题
- **修复** JFrog 获取项目列表时无权限的问题
- **修复** 通过最近运行状态查询流水线列表错误的问题
- **修复** 流水线运行 node 为空的问题
- **修复** 流水线运行 node 状态与流水线状态不一致的问题
...
- **新增** 流水线内置模板支持启用和禁用
- **新增** 流水线支持git类型参数
- **新增** 流水线运行记录列表支持多种过滤条件
- **新增** 工具链详情返回sonarqube在jenkins中的名称
- **新增** jforg工具链支持修改密码
- **优化** 全量同步流水线时jenkins压力过大的问题
...
- **新增** 支持删除废弃的工具链资源
- **新增** 支持通过 OpenAPI 和 Webhook 的方式传递运行流水线的上游流水线信息
- **新增** 支持 Jenkins `Extended Choice Parameter` 和 `Extensible Choice Parameter` 插件
- **新增** 重做流水线指标和面板
- **修复** 创建 GitOps 模块的控制面命名空间失败
- **修复** 流水线运行保留上传文件的名称
- **修复** 定时同步工具链的周期配置错误
- **修复** 更新 `sonarqube` 配置时，同时更新 Jenkins 中的凭据
- **修复** 同步工具链项目时会报唯一索引重复的问题
- **优化** ArgoCD 链接，减少不必要的 Token 生成
- **优化** `from-git` 和 `from-jar` 生成的流水线模板
...
- **新增** 支持修改集成的工具链配置
- **新增** 运行流水线时的参数校验能力
- **修复** `Rollout` 在开启监控分析后始终失败的问题
- **修复** 不必要的 `Gateway` 资源被创建的问题，该问题的影响：在开启 `Argo CD UI` 后重装 Ghippo 会导致 DCE5 页面无法访问
- **修复** 复制流水线会导致 Webhook 被重复触发的问题
- **修复** 复制流水线时，新流水线错误记录执行状态的问题
- **修复** 流水线审核中用户组权限未正确处理的问题
- **修复** Jenkinsfile 中定义的构建参数未被触发的问题
- **修复** 更新应用时别名和描述不生效的问题
- **优化** ArgoCD 的连接方式，提高稳定性
- **优化** 流水线密码类型参数的传递方式，增强安全性
- **优化** ArgoCD `destination`、`project`、`cluster` 的同步逻辑，使流程更健壮
...
- **新增** 流水线模板：部署到虚拟机
- **新增** `Argo CD UI` 和 `AppAnyNamespace` 两个 `FeatureGate`
- **新增** 流水线缓存配置可修改缓存容量大小（根据 `AllowVolumeExpansion` 字段判断是否支持更新）
- **修复** `ArgoRollouts` 监控面板的命名空间选择框没有根据集群过滤的问题
- **修复** `gitops event` 显示混乱的问题
- **修复** 在 `app of apps` 模式下，创建的 `Application` 应位于 ArgoCD 命名空间
- **修复** 默认集成的 Jenkins 缺少集群和命名空间信息
- **修复** 命名空间显示逻辑混乱的问题
- **修复** `rollouts` 更新镜像时修改其他字段的问题
...
- **新增** 管理员视图中流水线缓存配置支持选择存储类（StorageClass）
- **新增** 流水线缓存支持直接更新
...
- **新增** 支持在GitOps 应用中创建 `ApplicationSet` 资源
- **修复** 无权限审核流水线时导致重定向的问题
- **修复** 开启 `ApplicationSet` 功能后， `GitOps Dashboard` 无法访问的问题
- **优化** 支持在画布视图和 Jenkinsfile 视图之间切换
- **优化** GitOps 模块仓库导入支持配置高级参数
- **优化** 支持在 GitOps 应用中查看 Kubernetes 资源详情
- **优化** 流水线运行记录支持查看参数
...
- **新增** 支持全局流水线模版
- **新增** 支持流水线分组管理
- **修复** GitOps 在不同工作空间下创建相同 Git 仓库失败的问题（需 ArgoCD 升级至 v2.12.0，因为 Addon v0.21.0 离线包已提供该版本 Chart）
- **修复** 灰度发布获取云原生网关失败的问题
- **修复** `gitops-deploy` 渲染模板错误的问题
- **修复** 原生应用获取路由列表提示错误的问题
- **优化** 流水线 DAG 选择 v2 版本后，下次进入界面自动默认使用 v2
