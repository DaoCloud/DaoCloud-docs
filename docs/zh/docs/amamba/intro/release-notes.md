...
- **修复** 更新应用时别名和描述不生效的问题
- **优化** `Argo CD` 的连接方式，提高稳定性
- **优化** 流水线密码类型参数的传递方式，增强安全性
- **优化** `Argo CD` `destination`、`project`、`cluster` 的同步逻辑，使流程更健壮
...
- **新增** 支持在`GitOps` 应用中创建 `ApplicationSet` 资源
...
- **新增** 支持全局流水线模版
- **新增** 支持流水线分组管理
- **修复** `GitOps` 在不同工作空间下创建相同 Git 仓库失败的问题（需 `Argo CD` 升级至 v2.12.0，因为 Addon v0.21.0 离线包已提供该版本 Chart）
- **修复** 灰度发布获取云原生网关失败的问题
