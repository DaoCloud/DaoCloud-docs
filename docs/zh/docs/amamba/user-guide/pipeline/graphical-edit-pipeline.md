...
- 创建一条自定义流水线，请参考[创建自定义流水线](create-pipeline-byself.md)，其中创建自定义流水线时需要在构建参数中添加两个字符串参数，这些参数将用于镜像构建的命令中。参数说明如下：
...
!!! note
    ...
    镜像更新后还可以实现触发流水线，具体操作请参考[触发流水线](pipeline-trigger.md)
