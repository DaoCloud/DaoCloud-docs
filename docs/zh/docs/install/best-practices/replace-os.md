...
某客户生产环境在 CentOS7.9 操作系统上部署了 DCE5 工作集群，但是由于 CentOS7.9 将停止维护，由于是生产环境，客户希望在不停机的情况下，从 CentOS7.9 替换成 Ubuntu22.04。

## 风险

- k8s 集群节点的迁移最佳实践是先增后删的操作顺序，但客户要求保证节点 IP 不发生变化，基于此只能调整节点迁移顺序为先删后增，此顺序的改变会导致集群容错性及稳定性下降，需要谨慎对待！

- 另外先删后增的过程中，原节点上的资源会迁移到其他节点，应保证其他节点拥有足够资源和足够的 Pods 容纳能力，否则会导致迁移失败；

    - 默认单节点最大容纳 Pods 数 kubelet_max_pods 为 110, 若迁移可能导致超标，则最好提前更新所有节点的容纳能力

    - 删除过程中，如果有服务固定在移除节点 / 流量指向移除节点Pod，会有2-3s的业务断连

## 解决方案

- 需要从 control plane 、worker 节点两个角度来处理
- 迁移操作顺序：worker 节点迁移 → 非首个 control plane 节点迁移 → 首个 control plane 节点迁移
- 在进行节点替换操作之前，安全起见，建议对集群的相关资源先行备份
...
1. 通过安装器命令，导入 Ubuntu22.04 的 [iso](../commercial/start-install.md/#iso-操作系统镜像文件必需)、[ospackage](../commercial/start-install.md/#ospackage-离线包必需) 文件：
...
2. 配置 ubuntu22.04 离线源的示例（--limit 用于限制源配置仅作用于指定扩容的节点）：
...
2. 在`节点管理`列表中，选择一个工作节点，点击 `移除`

3. 移除成功后，在终端命令行中，连接 Global 集群，获取资源类型为 `clusters.kubean.io`，名称为 <工作集群名称> 下的参数 `hosts-conf` 信息，此示例工作集群名称为：centos
...
4. 在`节点管理`列表中，点击 `接入节点`，将之前移除的节点重新加入进来即可

6. 等待新节点扩容成功后，其他工作节点重复以上步骤直至全部替换完成

### Control Plane 节点替换

Control Plane 节点替换需要分成两部分，分别为 首个 Control Plane 节点替换、非首个 Control Plane 节点替换
...
```yaml
      ...
      children:
        kube_control_plane:
          hosts:
            centos-master1: null # 即为首个节点
            centos-master2: null
            centos-master3: null
```

#### 非首个 Control Plane 节点替换

1. 在终端命令行中，连接 Global 集群，获取资源类型为 `clusters.kubean.io`，名称为 <工作集群名称> 下的参数 `hosts-conf` 、`vars-conf` 信息，此示例工作集群名称为：centos
...
5. 部署上述文件后且等待第 4 步移除节点重新扩容进来后，重复执行第 3、4 步完成第三个节点的替换

#### 首个 Control Plane 节点替换

可参考文档[替换工作集群的首个控制节点](https://docs.daocloud.io/kpanda/best-practice/replace-first-master-node/)

1. 在容器管理集群对工作集群的 kubeconfig 进行更新，终端登录到第二个节点

2. 更新 `configmap` 资源 `centos-hosts-conf`，调整工作集群首个 Control Plane 节点在 kube_control_plane 、kube_node、etcd 中的顺序 （node1/node2/node3 -> node2/node3/node1）：
...
4. 部署上述文件且缩容成功后，更新 `configmap` 资源 cluster-info、kubeadm-config
    ...
    # 2. 需改 server 字段的 IP 地址为第二个 control plane IP
    ```
...
6. 扩容任务完成后即成功完成首个 Control Plane 节点替换
