...
!!! note
    ...
    - 若非上表中声明的操作系统，请参考文档[在其他 Linux 上离线部署](../os-install/otherlinux.md)进行安装部署。
...
| **服务器角色** | **磁盘分区/目录** | 空余容量建议 | 说明 |
| ------------- | ------------ | ----------- | ----------- | 
| all in one | /var/lib | **>50G** | Master 节点该路径承载本地镜像、 etcd、监控、日志等数据，需预留缓冲 ；Worker 节点实际占用较少，但建议统一标准。 |
| all in one | / (根文件系统) | **>70G** | 若 /home 和 /var/lib 不独立分区，需将其所需空间叠加到根分区。 |
| all in one | /home | **>150G** | / |
...
| 3 | 控制面 | 1. 运行 DaoCloud 产品组件<br />2. 运行 kubernetes 系统组件 | 16核 | 32 GB | 100 GB | 200 GB |

如果您需要对系统硬盘进行分区管理，请参考下表分配分区资源。

| **服务器角色** | **磁盘分区/目录** | 空余容量建议 | 说明 |
| ------------- | ------------ | ----------- | ----------- |
| 火种节点 | /var/lib | **>30G** | 主要用于火种 podman 容器存储 (/var/lib/containers)  |
| 火种节点 | / (根文件系统) | **>150G** | 存放离线安装资源（ISO、Addon、Binaries、Images 等） |
| 火种节点 | /home | **>50G** | 若 /home 和 /var/lib 不独立分区，需将其所需空间叠加到根分区。 |
| 火种节点（考虑升级） | /var/lib | **>30G** | / |
| 火种节点（考虑升级） | / (根文件系统) | **>50G** | / |
| 火种节点（考虑升级） | /home | **>300G** | 若需保留旧版本安装包以支持升级，空间需求翻倍 |
| 控制面 | /var/lib | **>50G** | /|
| 控制面 | / (根文件系统) | **>50G** | / |
...
| 3 | master | 1. 运行 DaoCloud 产品组件<br /> 2. 运行 kubernetes 系统组件 | 16 核 | 16 GB | 100 GB | 200 GB |
...
| **资源** | **要求** | **说明** |
| ------- | -------- | ------- |
| `istioGatewayVip` | 1 个 | 如果负载均衡模式是 metallb，则需要指定一个 VIP，提供 UI 界面和 OpenAPI 访问入口。 |
