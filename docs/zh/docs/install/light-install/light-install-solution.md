...
DCE 社区版部署轻量化裁剪方案和验证流程，旨在满足客户场景部署轻量化需求。
...
1. 关闭火种 kind-cluster 以及 elasticsearch 组件
...
1. 在全局管理集群中部署镜像仓库，通过 kangaroo 纳管 [可行]

1. 部署 MySQL 单实例，容器管理外接 MySQL 实例 [可行]

### 阶段优化

**Phase1 优化**

Phase1 裁剪 infrastructure & components 非必需组件。
...
故综合情况下，实际上 8G 内存环境中运行 DCE 轻量化裁剪环境，资源依然不足；
