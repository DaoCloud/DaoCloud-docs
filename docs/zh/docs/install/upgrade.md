...
产品组件的升级包含升级产品功能模块、升级基础设施模块。

- 产品功能模块由容器管理、全局管理、可观测性等十几个子模块构成，主要指
  [manifest.yaml](commercial/manifest.md) 文件中的 `components` 部分。
- 基础设施模块的组件特指 [manifest.yaml](commercial/manifest.md) 文件中的 `infrastructures` 部分。

!!! warning

    - 由于包含较多产品模块，所以使用安装器升级组件时，建议逐版本升级，请勿跨多个版本进行升级！
    - 升级组件可能会覆盖您的业务数据，请先备份好数据，重要！！！

## 前提条件

- 您需要有一个安装了对应产品的集群环境，参阅[离线化部署商业版](commercial/start-install.md)
- 请确保您的火种机器还存活
...
产品功能模块的组件特指 [manifest.yaml](commercial/manifest.md) 文件中的 `components` 部分。
...
基础设施模块的组件特指 [manifest.yaml](commercial/manifest.md) 文件中的 `infrastructures` 部分，如下配置就是基础设施中的 `hwameiStor` 组件：
...
- `install-app` 或 `cluster-create`，代表安装模式类型。如果最初的环境是通过 `cluster-create` 来安装的，则升级时也采用这个命令
...
!!! note
    ...
    - 对于 d.run Token 工厂，替换为 `-m ./offline/sample/manifest-cloud-tokfact.yaml`
    - 对于 d.run AI 操作系统，替换为 `-m ./offline/sample/manifest-cloud.yaml`
