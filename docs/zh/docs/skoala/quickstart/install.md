...
商业版的 **版本号 ≥ v0.3.29** 时，默认会安装微服务引擎，但仍旧建议检查 `mainfest.yaml` 文件进行确认
...
左图蓝色框内的 chart 即 `skoala` 组件，需要安装在控制面集群，即 DCE 5.0 的全局集群 `kpanda-global-clsuter`，
...
右图蓝色框内的 chart 即 `skoala-init` 组件，需要安装在工作集群。安装 `skoala-init`
