...
本文介绍 云原生网关在不同场景下的性能表现，便于您根据需求为网关配置合适的资源。
...
| 对象         | 角色                           |说明
| -------------- | ------------------------------ | ---|
| 云原生网关|测试对象|采用一主一从的模式，部署位置：172.30.120.211 |
|Locust|测试工具|采用 1+4 的主从分布式运行模式，四台压测机的资源配置均为 8 核 8 G|
|Nginx|用于测试网关性能的演示服务|通过 云原生网关访问，访问地址为：http://172.30.120.211:30296/|
| contour |  云原生网关的控制面 |版本为 1.23.1|
| envoy |  云原生网关的数据面  |版本为 1.24.0|
| 全局管理 | 云原生网关依赖的组件 |版本为 0.12.1|
| 容器管理 | 云原生网关依赖的组件 |版本为 0.13.1|
| 微服务引擎 | 云原生网关依赖的组件 |版本为 0.15.1|

## 性能指标

- 吞吐量（RPS）：每秒处理的请求数。结合 CPU 使用率，判断 云原生网关在特定资源配置下每秒可以处理的并发请求数。
  吞吐量越高，说明网关性能越好
- CPU 使用率：测试处理特定数量的并发请求时，云原生网关实例的 CPU 使用情况。
...
!!! success
    ...
    综上，当服务部署三副本且不限制资源用量时，云原生网关能够处理的最大并发请求数大约为 6000 到 7000，较同类产品而言可谓性能优异。
...
云原生网关是在开源项目 contour 和 envoy 的基础上进一步研发优化而来。contour 充当网关的控制平面，envoy 充当数据平面。

创建 云原生网关时，系统要求必须为网关配置不低于 1 核 1 G 的资源。因此，本次测试中 contour 的资源限制最低为 1 核 1 G。
