...
微服务引擎属于 DCE 高级版功能，其中包含注册中心、配置中心、微服务治理（传统微服务、云原生微服务）、云原生网关等功能。
...
本次实践使用的示例应用基于 OpenTelemetry 的标准演示应用，由 DaoCloud 大微服务团队根据 DCE
...
[应用工作台](../../amamba/intro/index.md)是 DCE 的应用管理模块，支持创建/维护多种类型的应用、GitOps
...
而 DCE 提供的注册中心完全兼容开源 Nacos，所以可以直接使用开源 Nacos 的 SDK。
...
基于 DCE 的特性，云原生网关可以自动发现所管辖命名空间内的服务，所以无需手动接入服务。
...
以上就是整个微服务引擎模块的体验之旅。在整个 DCE 的能力支持下我们顺利完成了应用部署、
启用微服务治理、配置并测试微服务治理策略、通过云原生网关开放 API、实际访问应用等操作。

### 更多能力

当我们的应用部署成功之后，对于后续的应用维护过程中，实际非常依赖 DCE 提供的可观测能力。接下来，我们会补充对应可观测能力实践。
