...
网卡、CPU 和内存未能正确对齐在同一个 NUMA 节点、PCIe 根或网络结构中时，性能可能会下降 30-50% 甚至更多。
...
![Device Plugin 管理的局限性](./images/topo03.png)
...
[Kueue](https://github.com/kubernetes-sigs/kueue) 通过节点标签提供拓扑感知调度。它使用分层拓扑级别：
...
代表了 Kubernetes 处理设备拓扑方式的根本性转变。DRA 提供结构化参数，实现丰富的拓扑表达和约束规范。
...
随着 AI 工作负载复杂性的持续增长，对精细拓扑感知调度的需求只会增加。无论你使用
Kueue、Volcano 还是原生 Kubernetes 调度，理解拓扑并规划 DRA 采用对于优化 AI 基础设施至关重要。
