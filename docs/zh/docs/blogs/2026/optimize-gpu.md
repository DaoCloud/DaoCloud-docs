...
在这方面，[HAMi](https://project-hami.io/zh/) 这类开源项目提供了 Kubernetes 原生的异构 GPU 调度与共享能力。
作为 CNCF 孵化项目，HAMi 支持按显存与算力精细切分 GPU 资源，并通过拓扑感知策略优化多 GPU 任务调度，已覆盖包括沐曦在内的多种国产 GPU 设备。
...
用户不应该因为底层 GPU 型号发生变化，就需要重新设计整个应用交付流程。这一目标的落地离不开开源社区的支撑，例如
[HAMi](https://project-hami.io/zh/) 将不同厂商 GPU 的设备接入、资源切分和监控指标统一在 Kubernetes
原生框架下，上层工作负载无需关心底层硬件差异，调度和隔离由 HAMi 在运行时完成。

目前 DaoCloud 已支持包括 NVIDIA、华为昇腾、天数智芯、沐曦、燧原、壁仞、寒武纪等在内的多种 GPU/NPU 设备，
覆盖驱动安装、设备插件、资源调度和监控等完整链路，详见
[GPU 支持矩阵](https://docs.daocloud.io/kpanda/user-guide/gpu/gpu_matrix/)。
