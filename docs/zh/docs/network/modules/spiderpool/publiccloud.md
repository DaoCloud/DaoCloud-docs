...
- 公有云的 VPC 网络中，处于网络安全管控和数据包转发的原理，当网络数据报文中出现 VPC 网络未知的 MAC 和 IP 地址时，它无法得到正确的转发。例如，基于 Macvlan 和 OVS 原理的 Underlay CNI 插件，Pod 网卡中的 MAC 地址是新生成的，会导致 Pod 无法通信。

    针对该问题，Spiderpool 搭配 [IPVlan](https://www.cni.dev/plugins/current/main/ipvlan/) CNI 进行解决。IPVlan 基于三层网络，无需依赖二层广播，并且不会重新生成 Mac 地址，与父接口保持一致，因此通过 IPvlan 可以解决公有云中关于 MAC 地址合法性的问题。

- 公有云下使用 Underlay 网络，但公有云的每个云服务器的每张网卡只能分配有限的 IP 地址，当应用运行在某个云服务器上时，需要同步获取到 VPC 网络中分配给该云服务器不同网卡的合法 IP 地址，才能实现通信。
...
helm install spiderpool spiderpool/spiderpool --namespace kube-system --set ipam.enableStatefulSet=false
```
...
在本文示例中，使用如上配置，创建两个 IPvlan SpiderMultusConfig，并自动生成如下的 Multus  NetworkAttachmentDefinition，他们分别对应了宿主机的 `eth0` 与 `eth1` 网卡。
...
1. 集群节点配置 `providerID`
    ...
    为集群中的 `每个节点` 配置 `providerID`，该步骤必须被执行，否则对应节点的 CCM Pod 将无法运行。
...
4. 创建 CCM 的 Cloud ConfigMap。
    ...
    将以下内容保存为 configmap-ccm.sh，然后通过 `./configmap-ccm.sh` 执行该脚本，完成创建。
...
Spiderpool 能够运行在任意的公有云集群中，并且可以保证集群的东西向与南北向流量均正常，为它们提供了一种统一的 Underlay 网络解决方案。
