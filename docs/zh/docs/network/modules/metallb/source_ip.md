#  通过Metallb + istio-ingressgateway 获取客户端源IP

## 背景

用户希望在 全景管理模块 >> 审计日志中能够查看操作者的真实IP,而不是被 SNAT 后的 IP 地址, 通过 Metallb ARP 模式 可以满足此需求。主要思路为: 将 istio-gateway pod部署的
...
4. 修改service: istio-ingressgateway 的 `spec.externalTrafficPolicy` = `Local` , 此模式可以保留真实源 IP:
