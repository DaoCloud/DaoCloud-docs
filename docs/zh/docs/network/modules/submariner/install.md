...
2. 在左侧导航栏中选择 `Helm 应用` -> `Helm 模板`，找到并点击 `submariner-k8s-broker`。
    ...
    ![](../../images/submariner-k8s-broker-helm-repo.png)

3. 在版本选择中选择希望安装的版本, 点击安装.

4. 推荐安装到 `submariner-k8s-broker` 命名空间下:

    ![](../../images/submariner-k8s-broker-ns.png)

5. 无需更改配置, 默认参数即可:

    ![](../../images/submariner-k8s-broker-config.png)

6. 在 Broker 集群成功安装 `submariner-k8s-broker`:

    ![](../../images/submariner-k8s-broker-install.png)

7. 切换到其中子集群: master1, 安装 `submariner-operator`:

    ![](../../images/submariner-operator-helm-repo.png)

8. 在版本选择中选择希望安装的版本, 点击安装

9. 推荐安装到 `submariner-operator` 命名空间下, 开启就绪等待:

    ![](../../images/submariner-operator-ns.png)

10. 配置 `submariner-operator` 连接 Broker 集群的配置:

    ![](../../images/submariner-operator-broker.png)
...
11. 配置 `submariner-operator` :
    ...
    ![](../../images/submariner-operator-install1.png)
     ...
     * _clusterCidr_: 填写子集群 Pod 的 CIDR。
     ...
     ![](../../images/submariner-operator-install2.png)
...
12. 在子集群 master1 安装完成:
     ...
     ![](../../images/submariner-opearator-install-3.png)

13. 安装完成后, 需要手动设置子集群 master1 其中某个节点为网关节点, 需要添加标签 "submariner.io/gateway: true" :

     ![](../../images/submariner-operator-gateway-label.png)
