...
- 当集群的 CNI 为 Calico 时, 由于 Calico 插入的 IPTables 规则优先级高于 Submariner 插入的 IPTables 规则, 导致跨集群通讯出现问题。所以我们必须手动配置 Calico 的
IPPool 来规避这个问题:
...
显示导出成功, 即可在ClusterB访问这个 Service:
