...
- **修复** 修复不同集群使用相同名称的minio创建托管harbor失败的问题
...
- **修复** 修复创建镜像空间，计数存在延迟问题
- **修复** 修复托管Harbor Chartmuseum 没有正确使用redis 问题
- **修复** 修复 GetRepository 接口和 ListRepository 接口返回值不一致问题
- **修复** 修复删除管理员集成 docker-registry镜像提示405错误问题
- **修复** 修复镜像漏洞问题
- **修复** 修复创建 Harbor Minio 实例会重启问题
...
- **修复** 修复托管 Harbor 编辑删除没有审计日志的问题
- **修复** 修复创建内置中间件接口拉取镜像报错
- **修复** 修复开启 OIDC 之后 Admin 不能通过 API 接口操作 Harbor 的问题

## 2024-07-31

### v0.20.0

- **修复** 修复部分镜像拉取失败导致的在线创建 Harbor 不成功问题
- **修复** 修复 Admin 用户权限点与自定义角色权限点数量不一致问题
- **修复** 修复部分权限溢出问题
