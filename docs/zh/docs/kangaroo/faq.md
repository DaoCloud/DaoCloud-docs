...
DCE5.0标准版本中没有中间件，中间件属于白金版。

## 如何校验配置的中间件网络是否可连接

登陆部署 Harbor 的目标集群，在任意几点中执行 `ping` 命令。
...
在使用中间件部署的Minio时，需要先手动通过Minio管理平台创建好bucket。

## 使用ingress暴露Harbor，本地访问不到

`ingress`暴露的`Harbor`需要使用域名进行访问，本地要能访问到需要本地能解析这个域名。
