...
- `Harbor Operator` 安装不成功需要检查这几点，`cert-manager`是否安装成功，`installCRDs` 是否设置为`true`. 安装`Harbor operator`的`helm`任务是否成功。
...
- 镜像仓库是严格按照`DEC5.0`的权限来执行的，在镜像仓库中某个用户必须要属于某个租户，才能看到当前租户下的私有镜像空间，否则即使管理员也不能看到。

## 私有镜像绑定工作空间后不能查询到

- 私有镜像绑定工作空间后程序需要异步执行很多逻辑，所以不会马上能看到，这个过程会收系统的影响，如果系统响应较快，则异步执行较快，一分钟内能看到。最长应该不会超过五分钟。

## 托管Harbor创建后能访问了但是状态依然不健康

- 目前托管Harbor页面上的状态和仓库集成的状态是二合一的，当两个状态都为健康的时候才是健康，因此可能出现托管`Harbor`已经可以访问了，但是状态依然不健康，这种情况需要等一个服务探测周期，一个探测周期是10分钟，在一个周期后就会恢复如初。
