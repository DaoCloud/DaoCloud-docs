...
| 类别 | 影响因素 | 风险级别 | 可能导致的风险 |
| ---- | ------- | ------ | ----------- |
| 未经保护的工作负载 | 工作负载未经 NAT、LB 等保护，直接向公网提供服务 | 高 | DCE 等资源直接绑定公网 IP 后会绕过一些安全设置，产生的安全防护和监控的盲点。若 DCE 上存在高危漏洞或端口暴露，则无法及时发现网络攻击和网络嗅探。|
...
| 数据安全 | 检查云产品资源实例在处理数据时，是否有进行数据访问控制和加密 | HwameiStor 存储开启服务端加密 |
| 日志审计 | 检查云产品及云平台是否开启日志审计功能 | 用户在访问 HwameiStor 的过程中，会产生大量的访问日志。您可以使用 DCE 5.0 或搭建 Spark 集群等方式对这些日志文件进行分析。|
...
- 使用安全模块的云平台配置检查功能对当前云账号下的默认配置及安全控制项进行检测。
- 通过查看云平台配置检查列表中的检测策略，
...
对于组织来说，不同职责的人需要访问云上不同类型的资源。安全管理员往往需要访问安全产品，
...
1. 高防和 WAF
2. 划分公网 vSwitch 和私网 vSwitch，VPC Ingress Routing
3. 网络 ACL
4. 安全组/IP 白名单
...
此外，根据零信任原则，不同 VPC、外部 IDC 等网络均属于位于的网络边界，在跨边界访问时，可以进行网络流量的隔离和防护。
...
在基础网络层面，可以详细记录 VPC 访问公网（南北向流量）、VPC 之间/混合云组网（东西向流量）的方位行为和异常流量。
...
1. 通过道客安全模块漏洞管理功能，自动识别云上的服务器和资产，需要通过配置漏洞扫描的任务设置，使其能够自动化的识别和检测漏洞。
...
安全模块是道客在云上原生的工作负载防护组件。
企业在云端创建的 DCE 默认安装安全模块 Agent，安全模块通过 Agent 对工作负载进行运行时防护。

通过安全模块可实时为工作负载提供运行时防护，包括服务器、容器以及云产品等。

1. 开启运行时防护：开启安全模块的运行时防护，通常情况下您需要确保创建 DCE 时，勾选安全加固选项，
   以保证安全模块的 Agent 自动部署，同时根据防护需求开通安全模块的功能授权即可。
2. 开启防病毒：配置防病毒检查策略，可以选择业务系统相关的服务器或全部服务器，并设定扫描周期。
3. 开启防勒索：配置防勒索功能，可以对您服务器进行勒索病毒攻击防护，安全模块通过勒索病毒检测、投放诱捕目录和设立重要数据备份功能起到防护效果。
4. 开启主机防御：配置主机防御规则，通过设定一系列的规则，如暴力破解的规则，自定义进程告警的规则等对主机上存在的异常行为进行识别和阻断。
5. 开启容器防御：配置容器主动防御规则，如设定未通过安全检测的容器镜像禁止启动等，设定容器文件防御等。
6. 查看运行时防护告警：通过安全模块查看运行时告警，道客云产品将告警根据 ATT&CK 攻击阶段进行了划分，可以更直观的展示攻击的路径和过程，以及攻击的详情，
...
云环境中，企业会将数据存储在 MySQL、云盘以及大数据平台中，数据识别的难点在于数据分散，且格式各不相同，数据特征也有所不同。
其次就是数据识别的规范和标准的不统一。
...
道客提供云上快速数据分类分级识别。数据安全模块为不同行业（例如金融、能源、汽车行业）提供了识别敏感数据的解决方案，即内置的识别模板。
使用识别模板可以检测您资产中是否存在敏感数据。可以直接使用内置的识别模板，和行业模板。
也可以基于内置模板自定义识别模板。

数据安全模块将采用基于内容识别、元数据（Meta 数据）识别以及词典识别的模式，结合正则表达式、包含、不包含等运算符进行敏感数据特征检测，
从而形成识别规则，更加灵活的进行数据特征检测。数据安全模块针对常见敏感数据类型提供了内置识别特征，并支持自定义识别特征。
...
数据静态存储在云上时，需要执行数据静态存储加密，通常情况下企业将数据存储在云盘，云数据库、NAS 等位置。
...
另一方面，针对自动化漏洞检测，道客提供安全模块服务，从 Linux 软件漏洞、Windows 系统漏洞、Web-CMS
...
安全模块关注多云场景下的主机、容器、虚拟机等工作负载的安全防护，提供云上资产管理、配置核查、主动防御、安全加固、云产品配置评估和安全可视化等核心能力，
...
- 操作审计：日常通过操作审计，可记录云账号下用户登录及资源访问操作，以此实现安全分析、入侵检测、资源变更追踪以及合规性审计。
  也可将账号下的行为事件下载或保存到日志服务 SLS 或对象存储，然后进行行为分析、安全分析、资源变更行为追踪和行为合规性审计等操作。
...
2. 网络攻击类事件：网络攻击类事件根据攻击的严重程度可以对攻击者 IP 进行自动化封禁，该场景可以配置自动化处置剧本，
   对网络攻击事件的攻击 IP 进行提取，并在负载均衡等产品上进行快速拦截。
...
为了解决单集群无法突破物理距离限制的问题，道客提出了应用多活方案。核心思路是对数据进行分片，通过自上而下的流量路由，
...
- **冗余存储**
    ...
    HwameiStor 提供本地冗余存储和同城冗余存储两种存储冗余类型，覆盖从单可用区到多可用区的数据冗余机制，以保证数据的持久性和可用性。
...
- **跨区域复制**
    ...
    道客存储产品跨区域复制（Cross-Region Replication）功能是跨不同数据中心（地域）的存储空间自动、异步（近实时）复制文件，
    将 Object 的创建、更新和删除等操作从源存储空间复制到不同区域的目标存储空间，实现跨区域容灾。如果用户对数据的安全性和可用性有极高的要求，对所有写入的数据，
    都希望在另一个数据中心显式地维护一份副本，以备发生特大灾难（如地震、海啸等）导致一个数据中心损毁时，还能启用另一个数据中心的备份数据。
...
- **版本控制**
    ...
    版本控制是针对存储空间级别的数据保护功能。开启版本控制后，针对数据的覆盖和删除操作将会以历史版本的形式保存下来。
    用户在错误覆盖或者删除对象后，能够将存储的数据恢复至任意时刻的历史版本。
    当用户删除数据后想要找回时，可使用版本控制功能，恢复已删除的数据。

- **定时备份**

    存储数据可以使用定时备份功能将存储空间内的文件定期备份到云备份中，
    当用户的数据意外丢失时，可通过云备份进行恢复。文件存储 NAS 支持与云备份无缝衔接，
    云备份在备份通用型 NAS 时不进行文件系统快照，而是基于高效的文件系统扫描机制。
...
- 分账账单是对“明细账单”数据的进一步展开说明。
  ...
  支持将分拆型云产品的费用拆分到各个分拆项，为企业内部分账提供参考数据。
...
比如存储（Object Storage Service）产品，可以按照标准型存储量、低频型计费存储量、归档型计费存储量、冷归档型计费存储量进行规划存储量。
...
企业客户开展互联网音视频、云相册、数据备份等多种业务场景下，对数据存储与管理有比较大的需求时，可以使用预留存储空间产品。
...
存储 提供标准、低频访问、归档、冷归档、深度冷归档多种存储类型，全面覆盖从热到冷的各种数据存储场景。
...
- 网络安全：进行网络安全防护，支持多账号安全管控，无需在每个账号重复购买。
...
| 存储 | 海量、高吞吐、安全、低成本，多种访问方式，99.9999999999% 数据可靠性 | 大规模数据分层存储、数据备份、网站托管、动静资源分离、CDN 加速 |
...
5. 网络设备：包括路由器、交换机等，监控其网络流量、带宽、延迟等指标。
...
道客拥有丰富的云数据库产品选项，涵盖关系型数据库、非关系型数据库、数据仓库、数据库生态工具，
可以为企业数据生产和集成、实时处理、分析与发现、开发与管理提供全链路生命周期的服务。
