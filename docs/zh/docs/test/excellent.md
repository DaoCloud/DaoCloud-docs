...
道客负责基础设施和物理设备（包括计算、存储和网络设备）的物理和硬件安全，并负责运行在 DCE 分布式云操作系统之上的虚拟化层和云产品层的安全。
...
4. 使用多因素验证。多因素认证 MFA（Multi Factor Authentication）是一种简单有效的安全实践，在用户名和密码之外再增加一层安全保护，
   用于登录道客云产品或进行敏感操作时的二次身份验证，以此保护您的账号更安全。
...
道客密钥管理服务提供了不同类型的密钥用于云产品加密和应用加密场景。
...
1. 链路冗余性：在混合云组网中需要至少两条线路进行冗余或者主备负载，当一条链路异常的时候可以切换到另外一条线路。实现链路冗余大体可以分为以下几类：
    ...
    - VPN 冗余：如果本地数据中心拥有多个本地网关设备（本地网关设备上均拥有公网 IP 地址），用户可以使用其中的 2 个本地网关设备分别与专有网络
...
为实现服务高可用的目标可以使用负载均衡产品。负载均衡代理多台后端真实的业务服务器对外服务，真实服务器不对外暴露，
...
为了向广大用户提供更稳定可靠的负载均衡服务，负载均衡已在大部分地域部署了多可用区以实现同地域下的跨机房容灾。
当主可用区出现故障或不可用时，负载均衡有能力在非常短的时间内（约 30 秒）切换到备可用区并恢复服务；
...
- **数据库备份**
    ...
    道客数据库中间件具备数据库的备份恢复能力，除了小部分海量数据产品需要手工开启备份恢复功能外，
