...
工作负载是指在云平台提供业务支撑的服务，如 DCE、Kubernetes、容器等，工作负载架构风险是指工作负载为了提供业务支持所选择的部署方式、
...
如企业级云上网络分区分域设计、云上同城/异地容灾网络设计、VLAN 子网划分设计东西向流量隔离和管控设计、
...
推荐企业使用多账号权限架构来管理云上资源。多账号权限架构帮助企业实现强隔离、降风险，应对企业多层级组织体系，
...
- **企业管理账号** ：用于多账号管理，在该账号中启用工作空间（Workspace）并构建账号树，
...
- 在企业管理账号中使用工作空间（Workspace）构建账号树，
  将其它账号作为 **成员账号** 纳入工作空间进行多账号统一管理。
...
| 容灾备份 | 检查云产品的数据备份策略是否得到有效配置和执行 | 在控制台定期备份文件，能够在数据丢失或受损时及时恢复文件。|
...
通常通过道客云操作系统的控制台、CLI、特定场景下的客户端等方式对云上的资源进行操作。

程序身份则代表应用程序或服务，往往是通过道客云操作系统的 OpenAPI 来访问云上的资源和数据。
...
道客云操作系统支持单点登录（Single Sign On，简称 SSO）。在道客云操作系统上，我们建议通过 SSO 的方式跟组织内的 IdP 进行集成，
...
云账号 AccessKey 等同于账号的 Admin 权限，也就是该账号内的完全管理权限，而且无法进行条件限制（例如：访问来源 IP 地址、访问时间等），
...
云上的权限管理是为了控制某个身份在什么条件下对哪些资源能够执行哪些操作。道客云操作系统上的授权方式分为以下几种：

1. 基于身份的授权：主要是指针对访问控制用户、用户组或角色进行授权。
2. 基于资源的授权：部分模块支持为某个特定资源绑定权限，
   支持向其他账号的访问控制用户授予访问权限，以及向匿名用户授予带特定 IP 条件限制的访问权限。
3. 管控策略（Control Policy）：管控策略是一种针对启用了工作空间（Workspace）（Resource Directory，简称 RD）的多账号组织，
   基于资源结构（资源夹或成员）的访问控制策略，可以统一管理工作空间（Workspace）各层级内资源访问的权限边界，建立企业整体访问控制原则或局部专用原则。管控策略只定义权限边界，并不真正授予权限。
...
在云上，建议通过道客云操作系统账号或资源组两种方式，区分不同业务应用的资源。如果企业使用多个道客云操作系统账号管理云资源，
则可以使用工作空间（Workspace）构建企业组织结构，对账号和资源进行集中、有序地管理，不同的业务应用按账号维度进行区分，
授权时应用范围选择整个云账号。如果企业使用一个道客云操作系统账号管理所有云资源，
...
对于云上有多个道客云操作系统账号的组织，可以基于工作空间（Workspace）管控策略，限制成员账号内的访问控制身份权限范围，
...
比如在 VLAN 中的数据库集群不应有互联网访问能力，且应该部署到没有到互联网的路由的 vSwitch 中。
...
- 将所有资源都创建在一个 VLAN 或者 vSwitch 中，不同业务之间没有隔离会导致安全暴露的影响范围扩大
...
1. 高防和防火墙
2. 划分公网 vSwitch 和私网 vSwitch， Ingress Routing
3. 网络 ACL
4. 安全组/IP 白名单
...
此外，根据零信任原则，不同 Switch、外部 IDC 等网络均属于位于的网络边界，在跨边界访问时，可以进行网络流量的隔离和防护。
...
在 VLAN 内部，可以通过 VLAN flowlog 来记录详细的五元组日志，通过分析 VLAN flowlog 日志，
发现不符合安全分层互访策略的流量，通过网络 ACL 或者安全组来进行安全策略的加固。
如果对于数据包的内容有进一步的监控审计要求，可以通过 VLAN 的流量镜像功能将流量复制一份，再导入流量分析组件进行更进一步的监控和分析。
...
在云环境中，密钥类别可分为通用密钥（数据库账号口令、服务器账号口令、SSH Key）和云账户密钥（AccessKey/SecretKey）。
...
- 网络管理：创建隔离分层的网络，有助于对相似的网络组件进行逻辑分组，还缩小了未经授权的网络访问的潜在影响范围。
  针对 VLAN，通过使用安全组、网络 ACL、流日志等，控制网络流量，保障云服务的安全性和可靠性，
  或是通过访问控制策略，对 VLAN 的访问权限进行控制。
...
- 入侵检测：通过检测入侵主要是为了防止数据泄露或者业务系统被破坏，通过配置相应的检测和告警机制，
  可了解数据中心或者云产品中存在的威胁，例如某个恶意 IP 对资产攻击、资产已被入侵的异常情况等。
...
- **定时备份**
    ...
    当用户的数据意外丢失时，可通过云备份进行恢复。文件存储支持与云备份无缝衔接，
    云备份在备份通用型存储时不进行文件系统快照，而是基于高效的文件系统扫描机制。
...
网络资源是指用于节点之间通信和数据传输的硬件和软件资源，除了包括广域网、局域网、交换机、路由器，还包括常用到的 VLAN、虚拟交换机、负载均衡、弹性公网、VPN、DNS、CDN 等资源。
...
块存储是基于分布式存储架构的云盘和基于物理机本地硬盘的本地盘产品。可以根据业务需求自定义云盘的预配置性能以及性能突发的能力。
...
对于有条件的企业，可以设立全国运行指挥中心（Global Operations Center，简称 GOC），实现 7*24 监控值班，时刻关注核心业务线上异常与故障。
...
选择云地域，在全国跨国企业的客户上云过程中，选择一个或者多个云区域，是一个重要的决策过程，涉及到成本需求的各个方面。
您在创建或者购买道客资源时需要选择部署地域，资源创建或购买成功后不能更换地域。选择地域可以考虑的因素有，
和客户自有 IDC 的连通需求可以选择就近的地域。业务终端客户所在的地域，比如某亚洲企业在本地和上海都有仓库，
上海的仓库的管理系统按照就近接入原则部署在道客云操作系统上海地域。同时地域的选择也要考虑地域之间网络专线和数据传输的成本。
...
1. 计费模式的选择：根据业务特性选择合适的计费方式是成本优化最直接的方式，从付费层面来看，按照业务场景长期使用的稳定业务资源需求通过选择成本较低的节省计划方式来支撑负载，
   ...
   如需长期运行，转预付费计费模式。无状态且可容错的业务可以通过成本非常低廉的实例来支撑。间歇性计算需求，和事件驱动的计算需求也可以使用道客函数计算产品。
...
许多云产品都会提供配套的资源包。资源包通常以预付费的方式购买更低折扣的抵扣权益，可以抵扣按量付费的云资源。
资源包加按量付费云资源的组合使用，通常可以节省更多成本。
...
道客资源实例规格族介绍了在售的所有实例规格族，包括每种实例规格族的特点、在售规格和适用场景。
...
如临时测试、弹性伸缩选择按量付费资源。无状态且可容错的业务可以通过成本非常低廉的实例来支撑。
...
也可以使用按量服务实例的方法应对无状态且有低算力需求的业务场景。
...
-实例配额按照资源供应确定性分为总配额、预留配额和保障配额。总配额为该实例的保有上限，保障配额为该实例的保障供应下限。
...
相比于随包年包月实例购买的云盘或者其他单一产品流量包，SCU 可以与多种云产品搭配使用，兼具了性价比与资源使用的灵活性。
...
专有容器集群需要做好集群宿主机的规格的规划，建议使用大规格实例：

使用大规格实例的优势：

- 网络带宽大，对于大带宽类的应用，资源利用率高。
- 容器在一个实例内建立通信的比例增大，减少网络传输。
- 拉取镜像的效率更高。因为镜像只需要拉取一次就可以被多个容器使用。
  而对于小规格的实例拉取镜像的次数就会增多，若需要联动实例伸缩集群，则需要花费更多的时间，反而达不到立即响应的目的。
...
企业上云后的财务管理也会因云计算的特性而发生改变，在用云过程中，
可以使用道客提供的财务管理工具对钱、账、票和云上财务资产实现多组织跨部门的高效管理。

##### 使用收支明细管理资金

企业用户需要在道客云操作系统上开通账户并进行企业认证。
...
- 月账单是用户需要跟道客进行实际结算的账单数据，月账单结合“月账单详情”可生成含道客财务专用章用印的 PDF 作为报销凭据，
  支持客户在多个账期中指定对某个账期的月账单单独进行还款。
...
道客资源管理服务是一系列企业 IT 治理产品和服务的集合，主要包括工作空间（Workspace）、资源组、资源共享和标签。您可以使用工作空间（Workspace）在云上构建企业业务组织关系，
...
利用道客 API 和自动化的能力，以编程方式动态改变架构中云资源的数量也是推荐的做法，
在需求高峰期间自动增加资源数量以保持业务，也可以在需求量降低时减少容量以降低成本。
运维编排服务是道客提供的云上自动化运维服务，能够自动化管理和执行任务。
企业可以通过模板来定义执行任务、执行顺序、执行输入和输出，然后通过执行模板来完成任务的自动化运行。
比如，如果业务系统在每天都有特定的流量高峰期时间段，在此时间段内需要大量的实例。
当过了每日的流量高峰期时间段后，可以使用 OOS 实现 DCE 定时开关机以节省成本。
...
道客通过可量化的实例规格、镜像、云盘、安全组等资源的数量限制和操作次数，比如 API 配速的限制来优化云平台全局资源的合理分配。
...
云产品的购买和使用不一定在同一时间，比如预付费购买 1 年的实例可以使用 12 个月，比如预付费购买的资源包在实际抵扣时才被消耗，
...
企业借助工作空间（Workspace）轻松实现企业跨道客账号资源的统一监控。对于混合云场景，建议使用企业级 Insight 监控立足云上，高效、低成本解决云下资源的监控数据的汇聚，
...
企业可以在网络计费方式优化中查看根据企业历史消费数据和优化建议算法得出的切换到系统的成本节省情况。

系统为用户提供了财务上的便利，比如用户可以通过系统账单方便的看到总体用量和费用。进而方便的掌握整体的使用量和费用数据。

云上流量成本的控制可以结合资源包，和阶梯定价的方式，系统测算之后的消费金额为流量包抵扣后的应付金额。

##### 调整资源包等抵扣资源降低按量付费成本

抵扣资源，指预留实例券、存储容量单位包、计算包，资源包等各类可以抵扣相关按量付费使用量的资源。
资源实例的售卖单价会比按量付费的单价低，通过购买抵扣资源可以降低按量付费的使用成本。

##### 跟踪最新的折扣权益和计费方式

道客会持续跟踪企业业务应用的用云需求，根据不同的业务场景提供细粒度的差异性实例，并在成本核算、计费方式上给企业用户提供便利。
建议持续跟踪道客最新的折扣权益，产品规格升级，新的工具和工具升级，比如系统成本优化建议后续会陆续支持更多类型的云产品，帮助客户进一步管理流量费用。
...
您可以像使用物理硬盘一样格式化并建立文件系统来使用块存储。道客文件存储是可以提供共享访问、弹性扩展、高可靠、高性能的本地存储系统。
可支持上千台弹性计算、容器服务等计算节点共享访问，您无需修改应用程序，即可无缝迁移业务系统上云。
...
有时用户会设置一个通用的运维管理方案，对于所有账号下集群按照标准统一管理。快照的计费基础是快照容量，
...
当使用网络服务时，在架构优化层面可以尽量使用内网进行应用间的通信；在做跨账号或跨 VLAN 之前的流量互通时规划好跨地域跨国家之间的网络产品。
...
弹性伸缩 Auto Scaling 是一个免费的服务，但是通过弹性伸缩自动创建的实例，需要按照 DCE 按量付费类型的实例标准进行付费。
...
- 弹性供应：一键部署跨付费模式、跨可用区和跨实例规格的实例集群。适合需要快速交付稳定算力并需要降低成本的场景。
...
例如，运维人员通过控制台创建、配置和监控集群、数据库、存储等资源，查看和处理系统告警和日志，业务人员可以通过控制台访问应用程序和查看业务数据等等。
...
可以指定实例的类型为实例。弹性伸缩作为云上广泛应用的功能提供多种伸缩模式，
...
在公网场景中，如海外流量在公网回中国内地的场景，因为跨境公网质量差导致了拥塞丢包，同时因为无直连回国公网线路导致了延迟高。

在就近访问场景中，可以利用 CDN 将源站资源缓存到道客遍布全国的加速节点，当终端用户请求访问和获取源站资源时无需回源，
可就近获取 CDN 节点上已经缓存的资源，提高资源访问速度，同时分担源站压力。

##### 混合云网络优化

当用户需要打通本地数据中心 IDC /总部/分支/移动终端等和 VLAN 来实现私网互访时，通常有高速通道、VPN 和智能接入网关等几种解决方案。
整体来看高速通道建立了高速、稳定、安全的私网通信，从性能层面无疑是最佳选择，但由于几种方案适用的场景差异性比较大，简单地横向比较在实践中的意义并不大，
具体情况还是需要结合具体场景进行选择。作为连接 IDC 与 VLAN 的可选项，基于物理专线的高速通道单链路最大支持 100 Gbps 的带宽，
