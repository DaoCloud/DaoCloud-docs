...
* Insight
    ...
    In DCE 5.0 v0.34.x version, Insight upgrades Jaeger from v1 to v2, with corresponding changes in the deployment architecture. 
    Before upgrading to this version, add the configuration item `jaeger.collector.enabled: false` under the 
    `components.insight.variables` field in the Manifest file manifest.yaml.
