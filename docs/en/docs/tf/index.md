...
Token Factory is an integrated platform built by DaoCloud for AI intelligent computing scenarios, providing end-to-end capabilities around large model inference, compute scheduling, and cluster O&M.
The platform adopts a **User** and **Administrator** dual-view architecture: the User view serves end users, providing large model service invocation, cost management, and intelligent application access capabilities; the Administrator view serves O&M management teams, providing unified management of model hosting, inference engines, infrastructure, and global operations.
...
As a high-efficiency AI Token production and management system designed for intelligent computing centers, Token Factory helps traditional computing centers upgrade to an efficient, profitable Token production and operation model.
The platform unifiedly manages NVIDIA and domestic heterogeneous computing power, relying on intelligent scheduling and inference optimization to transform distributed GPU compute into low-cost, highly stable, and tradable standardized Token services.
Built on the Token Factory manager and unified operations system, it achieves global management of resources, production, costs, and supply-demand, providing capabilities to end users, administrators, operators, and O&M teams,
driving intelligent computing centers to evolve from "providing compute" to "producing and operating Tokens".

## Dashboard - Operations Center

![Token Factory Operations Center](./images/tf01.png)

The Token Factory Operations Center is the intelligent production dashboard of the AI era, transforming the entire Token production chain into quantifiable, trackable, and optimizable real-time data views,
giving operations teams complete visibility into every second of the platform's operation. The dashboard presents real-time platform status with millisecond-level data refresh.
