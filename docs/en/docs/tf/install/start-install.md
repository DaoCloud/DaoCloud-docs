# Offline Install d.run Token Factory

This page describes how to install **d.run Token Factory (hereinafter referred to as Token Factory)** offline. Use the manifest file `manifest-cloud-tokfact.yaml` during installation.
...
The complete Token Factory offline package is available at [Download Center - Token Factory](../../download/tf/index.md).

### Offline Image Package (Required)

The offline image package contains the configuration files, image resources, and chart packages required for installing Token Factory.
You can download the latest version from the [Download Center - Token Factory](../../download/tf/index.md).
...
`osPackagePath` in [clusterConfig.yaml](../../install/commercial/cluster-config.md).
...
1. Run the following command to start installing Token Factory. The installer binary file is located at `offline/dce5-installer`. For the manifest, use `offline/sample/manifest-cloud-tokfact.yaml` in the offline package.
    ...
    !!! note
        ...
        - Use `-m` to specify the manifest file. For Token Factory, use `manifest-cloud-tokfact.yaml`.
    ...
    The following is an example of `manifest-cloud-tokfact.yaml` (fields and versions are subject to the extracted offline package; this is only to illustrate the structure):
    ...
    The core fields of the Token Factory manifest are described as follows:

    - `global.installMode` is set to `cloud`.
    - The core component enables the `global.tokenfactory.enable: true` parameter.
    - LLM Studio (`hydra`) and Cockpit (`crane`) are enabled by default.
...
3. After successfully installing Token Factory, contact us for authorization: email info@daocloud.io or call 400 002 6898.
