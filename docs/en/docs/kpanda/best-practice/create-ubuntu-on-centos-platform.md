...
!!! note
    ...
    This page is specifically for the offline mode, using the DCE platform to create a worker cluster,
...
- A fully deployed DCE system, with the bootstrap node still active. For deployment reference, see the documentation [Offline Install DCE Enterprise](../../install/commercial/start-install.md).

## Download and Import Ubuntu Offline Packages

Please ensure you are logged into the bootstrap node! Also, make sure that the
clusterConfig.yaml file used during the DCE deployment is still available.
