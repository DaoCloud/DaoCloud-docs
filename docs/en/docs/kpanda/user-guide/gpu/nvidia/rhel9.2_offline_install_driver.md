# Offline Installation Guide for GPU Operator Driver on RHEL 9.2

**Prerequisite**: GPU Operator version `v23.9.0+2` or later must already be installed.

The official NVIDIA driver image for RHEL 9.2 cannot be directly installed. There is a known issue with the official driver script. Until it is fixed, follow the steps below to install the driver offline.

## Disable Nouveau Driver

RHEL 9.2 comes with an unofficial `nouveau` NVIDIA driver, which needs to be disabled first.
...
# Reboot the VM
sudo reboot
# Check if Nouveau is successfully disabled
lsmod | grep nouveau
```

## Create a Custom Driver Image

First, create a `nvidia-driver` script file locally:

<details>
<summary>Click to view the full contents of the `nvidia-driver` script</summary>
...
Use the official image to rebuild a custom image. Below is the content of a sample `Dockerfile`:
...
Build and push the image to your local registry (e.g., Firefly cluster):

```bash
docker build -t {your-firefly-registry}/nvcr.m.daocloud.io/nvidia/driver:535.183.06-01-rhel9.2 -f Dockerfile .
