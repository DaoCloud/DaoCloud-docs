...
# Integrate DCE into Customer System (OEM OUT)

OEM OUT refers to integrating DCE as a sub-module into other products, appearing in their menus.
You can directly access DCE without logging in again after logging into other products.
...
1. Deploy DCE (Assuming the access address after deployment is `https://10.6.8.2:30343/`).

2. To achieve cross-domain access between the customer system and DCE, you can use an nginx reverse proxy.
...
3. Assuming the nginx entry address is 10.6.165.50, follow the
   [Customize DCE Reverse Proxy Server Address](../install/reverse-proxy.md) to
   set the DCE_PROXY reverse proxy as `http://10.6.165.50/dce5`. Ensure that DCE
...
Integrate the customer system with DCE using protocols like OIDC/OAUTH,
allowing users to access DCE without logging in again after logging into
...
After integration, the DCE login page will display the OIDC (custom) option.
When accessing DCE from the customer system for the first time,
select OIDC login, and subsequent logins will directly enter DCE without needing to choose again.

## Navigation Bar Integration

Navigation bar integration means adding DCE to the menu of the customer system.
You can directly access DCE by clicking the proper menu item. The navigation bar
