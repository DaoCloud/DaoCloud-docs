...
Most are new features. **This version contains many important functions and user experience optimization** . This page will introduce some important features in detail in the next section.
...
One of the most common cases for this function is: batch scheduling, where a group of Pods need to wait for the cluster resources to satisfy the entire group of Pods to start at the same time before scheduling.
...
In this release, DaoCloud mainly contributed **sig-node, sig-scheduling and kubeadm related content**, the specific functions are as follows:
...
- [Node] Improve the current Memory QoS function and optimize its adaptability in cgroup v2 cases
