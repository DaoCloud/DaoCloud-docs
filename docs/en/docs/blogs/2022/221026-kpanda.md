...
- Supports full lifecycle management of nodes in the management cluster and the worker cluster created based on the management cluster.
...
- **Elastic Scaling**: Through the interface, the manual/automatic scaling capability of the workload can be realized, and the scaling policy can be customized to cope with traffic peaks.
