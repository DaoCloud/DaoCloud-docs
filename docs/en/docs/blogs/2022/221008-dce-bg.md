...
For application developers, DCE 5.0 can be regarded as a cluster operating system. DCE provides features such as service discovery, scaling, load balancing, self-healing and even election, free developers from infrastructure-related configuration. DCE 5.0 can treat a large number of servers as a huge server and run applications on a large server.
