...
What is Cloud Native? The English name is Cloud Native, which is a concept proposed by Matt Stine, a veteran in the IT industry, including DevOps, CI/CD, microservices, and containerization.
...
DCE adopts industry-leading container orchestration technology, and its product capabilities cover the development, delivery, operation and maintenance of cloud native applications, as well as the lifecycle management of operations.
...
For application developers, DCE can be regarded as a cluster operating system. DCE provides features such as service discovery, scaling, load balancing, self-healing and even election, free developers from infrastructure-related configuration. DCE can treat a large number of servers as a huge server and run applications on a large server.
...
Learn more about [What is DCE](../../dce/index.md)
