...
There was no standard format to expose the health information of Kubernetes components before. This function adds a new path /metrics/slis to each component to expose the service level metric (ServiceLevelIndicator) in the Prometheus format.
